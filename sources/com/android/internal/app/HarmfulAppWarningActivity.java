...
public class HarmfulAppWarningActivity extends AlertActivity implements DialogInterface.OnClickListener {
    ...
    private View createView(ApplicationInfo applicationInfo) {
        View view = getLayoutInflater().inflate((int) R.layout.harmful_app_warning_dialog, (ViewGroup) null);
        ((TextView) view.findViewById(R.id.app_name_text)).setText(applicationInfo.loadSafeLabel(getPackageManager(), 500.0f, 5));
        ((TextView) view.findViewById(16908299)).setText((CharSequence) this.mHarmfulAppWarning);
        return view;
    }
    ...
    public void onClick(DialogInterface dialog, int which) {
        switch (which) {
            case -2:
                getPackageManager().setHarmfulAppWarning(this.mPackageName, (CharSequence) null);
                try {
                    startIntentSenderForResult((IntentSender) getIntent().getParcelableExtra(Intent.EXTRA_INTENT), -1, (Intent) null, 0, 0, 0);
                ...
                return;
            case -1:
                getPackageManager().deletePackage(this.mPackageName, (IPackageDeleteObserver) null, 0);
