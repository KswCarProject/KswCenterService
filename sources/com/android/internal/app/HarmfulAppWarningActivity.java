...
public class HarmfulAppWarningActivity extends AlertActivity implements DialogInterface.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            p.mTitle = getString(C3132R.string.harmful_app_warning_title);
            p.mView = createView(applicationInfo);
            p.mPositiveButtonText = getString(C3132R.string.harmful_app_warning_uninstall);
            p.mPositiveButtonListener = this;
            p.mNegativeButtonText = getString(C3132R.string.harmful_app_warning_open_anyway);
            p.mNegativeButtonListener = this;
            this.mAlert.installContent(this.mAlertParams);
        } catch (PackageManager.NameNotFoundException e) {
            Log.m69e(TAG, "Could not show warning because package does not exist ", e);
            finish();
        }
    }

    private View createView(ApplicationInfo applicationInfo) {
        View view = getLayoutInflater().inflate(C3132R.layout.harmful_app_warning_dialog, (ViewGroup) null);
        ((TextView) view.findViewById(C3132R.C3134id.app_name_text)).setText(applicationInfo.loadSafeLabel(getPackageManager(), 500.0f, 5));
        ...
        switch (which) {
            case -2:
                ...
                try {
                    ...
                    Log.m69e(TAG, "Error while starting intent sender", e);
                }
