...
public class MessagingImageMessage extends ImageView implements MessagingMessage {
    ...
    public MessagingImageMessage(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mPath = new Path();
        this.mMinImageHeight = context.getResources().getDimensionPixelSize(C3132R.dimen.messaging_image_min_size);
        this.mMaxImageHeight = context.getResources().getDimensionPixelSize(C3132R.dimen.messaging_image_max_height);
        this.mImageRounding = context.getResources().getDimensionPixelSize(C3132R.dimen.messaging_image_rounding);
        this.mExtraSpacing = context.getResources().getDimensionPixelSize(C3132R.dimen.messaging_image_extra_spacing);
        setMaxHeight(this.mMaxImageHeight);
        this.mIsolatedSize = getResources().getDimensionPixelSize(C3132R.dimen.messaging_avatar_size);
    }
    ...
    public boolean setMessage(Notification.MessagingStyle.Message message) {
        ...
        try {
            ...
            if (intrinsicHeight == 0) {
                Log.m64w(TAG, "Drawable with 0 intrinsic height was returned");
                return false;
            }
    ...
    static MessagingMessage createMessage(MessagingLayout layout, Notification.MessagingStyle.Message m, ImageResolver resolver) {
        ...
        if (createdMessage == null) {
            createdMessage = (MessagingImageMessage) LayoutInflater.from(layout.getContext()).inflate(C3132R.layout.notification_template_messaging_image_message, (ViewGroup) messagingLinearLayout, false);
