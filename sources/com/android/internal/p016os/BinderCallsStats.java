...
import com.android.internal.p016os.BinderCallsStats;
import com.android.internal.p016os.BinderInternal;
import com.android.internal.p016os.CachedDeviceState;
...
public class BinderCallsStats implements BinderInternal.Observer {
    ...
    @Override // com.android.internal.p016os.BinderInternal.Observer
    ...
    public ArrayList<ExportedCallStat> getExportedCallStats() {
        ...
        while (it.hasNext()) {
            ...
            if (0 != 0 && previous.className.equals(exported2.className)) {
                isClassDifferent = false;
            } else {
                isClassDifferent = true;
            }
            if (isClassDifferent) {
    ...
    private void dumpLocked(PrintWriter pw, AppIdToPackageMap packageMap, boolean verbose) {
        ...
        pw.println();
        pw.println(String.format("  Summary: total_cpu_time=%d, calls_count=%d, avg_call_cpu_time=%.0f", Long.valueOf(totalCpuTime), Long.valueOf(totalCallsCount2), Double.valueOf(totalCpuTime / totalRecordedCallsCount)));
        pw.println();
    ...
    public void setMaxBinderCallStats(int maxKeys) {
        if (maxKeys <= 0) {
            Slog.m50w(TAG, "Ignored invalid max value (value must be positive): " + maxKeys);
            return;
        }
        synchronized (this.mLock) {
    ...
    public void setSamplingInterval(int samplingInterval) {
        if (samplingInterval <= 0) {
            Slog.m50w(TAG, "Ignored invalid sampling interval (value must be positive): " + samplingInterval);
            return;
        }
        synchronized (this.mLock) {
