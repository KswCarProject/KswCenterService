...
public class SmsMessage extends SmsMessageBase {
    ...
    public static SmsMessage createFromPdu(byte[] pdu) {
        ...
        try {
            ...
            Log.m69e(LOG_TAG, "SMS PDU parsing failed with out of memory: ", e);
            return null;
    ...
    public static int getTPLayerLengthForPDU(String pdu) {
        Rlog.m80w(LOG_TAG, "getTPLayerLengthForPDU: is not supported in CDMA mode.");
        return 0;
    }
    ...
    public int getProtocolIdentifier() {
        Rlog.m80w(LOG_TAG, "getProtocolIdentifier: is not supported in CDMA mode.");
        return 0;
    }

    @Override // com.android.internal.telephony.SmsMessageBase
    public boolean isReplace() {
        Rlog.m80w(LOG_TAG, "isReplace: is not supported in CDMA mode.");
        return false;
    }

    @Override // com.android.internal.telephony.SmsMessageBase
    public boolean isCphsMwiMessage() {
        Rlog.m80w(LOG_TAG, "isCphsMwiMessage: is not supported in CDMA mode.");
        return false;
    }
    ...
    public boolean isReplyPathPresent() {
        Rlog.m80w(LOG_TAG, "isReplyPathPresent: is not supported in CDMA mode.");
        return false;
    }
    ...
    private void parsePdu(byte[] pdu) {
        ...
        try {
            ...
            Rlog.m86e(LOG_TAG, "createFromPdu: conversion from byte array to object failed: " + ex2);
        }
    ...
    private void parsePduFromEfRecord(byte[] pdu) {
        ...
        try {
            ...
            while (dis.available() > 0) {
                ...
                switch (parameterId) {
                    case 0:
                        ...
                        Rlog.m84i(LOG_TAG, "teleservice = " + env.teleService);
                        break;
                    case 1:
                        ...
                        if (addr.digitMode == 0) {
                            ...
                            while (true) {
                                ...
                                if (index2 < addr.numberOfDigits) {
                                    ...
                                    data[index2] = convertDtmfToAscii(b);
                                    index = index2 + 1;
                                }
                            }
                        } else if (addr.digitMode != 1) {
                            Rlog.m86e(LOG_TAG, "Incorrect Digit mode");
                            ...
                            if (numberType == 2) {
                                Rlog.m86e(LOG_TAG, "TODO: Addr is email id");
                            } else {
                                Rlog.m86e(LOG_TAG, "TODO: Addr is data network address");
                            }
                        } else {
                            Rlog.m86e(LOG_TAG, "Addr is of incorrect type");
                        }
                        addr.origBytes = data;
                    ...
                    case 5:
                        ...
                        for (int index5 = 0; index5 < subAddrLen; index5++) {
                            ...
                            subdata[index5] = convertDtmfToAscii(b3);
                        }
            ...
            Rlog.m86e(LOG_TAG, "parsePduFromEfRecord: conversion from pdu to SmsMessage failed" + ex);
        }
        this.mOriginatingAddress = addr;
        env.origAddress = addr;
        env.origSubaddress = subAddr;
    ...
    public void parseSms() {
        ...
        if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
            Rlog.m88d(LOG_TAG, "MT raw BearerData = '" + HexDump.toHexString(this.mEnvelope.bearerData) + "'");
            StringBuilder sb = new StringBuilder();
            sb.append("MT (decoded) BearerData = ");
            sb.append(this.mBearerData);
            Rlog.m88d(LOG_TAG, sb.toString());
        }
        ...
        if (this.mBearerData.messageType == 4) {
            if (!this.mBearerData.messageStatusSet) {
                ...
                sb2.append(" userData).");
                Rlog.m88d(LOG_TAG, sb2.toString());
                this.status = 0;
            } else {
    ...
    public SmsCbMessage parseBroadcastSms(String plmn) {
        ...
        if (bData == null) {
            Rlog.m80w(LOG_TAG, "BearerData.decode() returned null");
            return null;
        }
        if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
            Rlog.m88d(LOG_TAG, "MT raw BearerData = " + HexDump.toHexString(this.mEnvelope.bearerData));
    ...
    public static synchronized int getNextMessageId() {
        ...
        synchronized (SmsMessage.class) {
            ...
            try {
                ...
                if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
                    Rlog.m88d(LOG_TAG, "next persist.radio.cdma.msgid = " + nextMsgId);
                    Rlog.m88d(LOG_TAG, "readback gets " + SystemProperties.get(TelephonyProperties.PROPERTY_CDMA_MSG_ID));
                }
            } catch (RuntimeException ex) {
                Rlog.m86e(LOG_TAG, "set nextMessage ID failed: " + ex);
            }
        }
        return msgId;
    }
    ...
    private static SubmitPdu privateGetSubmitPdu(String destAddrStr, boolean statusReportRequested, UserData userData, int priority) {
        ...
        if (Rlog.isLoggable(LOGGABLE_TAG, 2)) {
            Rlog.m88d(LOG_TAG, "MO (encoded) BearerData = " + bearerData);
            Rlog.m88d(LOG_TAG, "MO raw BearerData = '" + HexDump.toHexString(encodedBearerData) + "'");
        }
        ...
        try {
            ...
            Rlog.m86e(LOG_TAG, "creating SubmitPdu failed: " + ex);
            return null;
        }
    }
    ...
    public void createPdu() {
        ...
        try {
            ...
            Rlog.m86e(LOG_TAG, "createPdu: conversion from object to byte array failed: " + ex);
        }
    }
    ...
    protected boolean processCdmaCTWdpHeader(SmsMessage sms) {
        try {
            ...
            if (subparamId != 0) {
                Rlog.m86e(LOG_TAG, "Invalid WDP SubparameterId");
                return false;
            }
            int subParamLen = inStream.read(8);
            if (subParamLen != 3) {
                Rlog.m86e(LOG_TAG, "Invalid WDP subparameter length");
                return false;
            }
            ...
            if (sms.mBearerData.hasUserDataHeader) {
                Rlog.m86e(LOG_TAG, "Invalid WDP UserData header value");
                return false;
            }
            inStream.skip(3);
            ...
            inStream.read(8);
            int subParamLen2 = inStream.read(8) * 8;
            ...
            if (sms.mBearerData.userData.msgEncoding == 0) {
                ...
                int remainingBits = subParamLen2 - consumedBits;
            ...
            Rlog.m86e(LOG_TAG, "Invalid WDP encoding");
            return false;
        } catch (BitwiseInputStream.AccessException ex) {
            Rlog.m86e(LOG_TAG, "CT WDP Header decode failed: " + ex);
            return false;
        }
    }
}
