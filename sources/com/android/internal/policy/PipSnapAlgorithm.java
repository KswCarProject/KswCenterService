...
public class PipSnapAlgorithm {
    ...
    public PipSnapAlgorithm(Context context) {
        ...
        this.mMinimizedVisibleSize = res.getDimensionPixelSize(C3132R.dimen.pip_minimized_visible_size);
        this.mDefaultSizePercent = res.getFloat(C3132R.dimen.config_pictureInPictureDefaultSizePercent);
        this.mMaxAspectRatioForMinSize = res.getFloat(C3132R.dimen.config_pictureInPictureAspectRatioLimitForMinSize);
        this.mMinAspectRatioForMinSize = 1.0f / this.mMaxAspectRatioForMinSize;
        this.mFlingDeceleration = this.mContext.getResources().getDimensionPixelSize(C3132R.dimen.pip_fling_deceleration);
    ...
    public void onConfigurationChanged() {
        ...
        this.mSnapMode = res.getInteger(C3132R.integer.config_pictureInPictureSnapMode);
    ...
    public Rect findClosestSnapBounds(Rect movementBounds, Rect stackBounds, float velocityX, float velocityY, Point dragStartPosition) {
        ...
        intersectStackBounds.offsetTo(intersect.f59x, intersect.f60y);
    ...
    public Point getEdgeIntersect(Rect stackBounds, Rect movementBounds, float velX, float velY, Point dragStartPosition) {
        ...
        vertPoint.f59x = velX > 0.0f ? movementBounds.right : movementBounds.left;
        vertPoint.f60y = findY(slope, yIntercept, vertPoint.f59x);
        horizPoint.f60y = velY > 0.0f ? movementBounds.bottom : movementBounds.top;
        horizPoint.f59x = findX(slope, yIntercept, horizPoint.f60y);
        if (isLandscape) {
            if (velX > 0.0f) {
        ...
        if (maxDistance > 0) {
            int startPoint = isLandscape ? dragStartPosition.f60y : dragStartPosition.f59x;
            int endPoint = isLandscape ? horizPoint.f60y : horizPoint.f59x;
            int center = movementBounds.centerX();
            if ((startPoint < center && endPoint < center) || (startPoint > center && endPoint > center)) {
                int distance = Math.min(((int) (0.0d - Math.pow(isLandscape ? velX : velY, 2.0d))) / (this.mFlingDeceleration * 2), maxDistance);
                if (isLandscape) {
                    horizPoint.f59x = stackBounds.left + (velX > 0.0f ? distance : -distance);
                } else {
                    horizPoint.f60y = stackBounds.top + (velY > 0.0f ? distance : -distance);
                }
                return horizPoint;
            }
        }
        double distanceVert = Math.hypot(vertPoint.f59x - x, vertPoint.f60y - y);
        double distanceHoriz = Math.hypot(horizPoint.f59x - x, horizPoint.f60y - y);
    ...
    public Rect findClosestSnapBounds(Rect movementBounds, Rect stackBounds) {
        ...
        if (this.mSnapMode == 4 || this.mSnapMode == 3) {
            ...
            if (distance < thresh) {
                newBounds.offsetTo(snapTarget.f59x, snapTarget.f60y);
            } else {
            ...
            newBounds.offsetTo(snapTarget2.f59x, snapTarget2.f60y);
        }
        return newBounds;
    }
    ...
    public void applyMinimizedOffset(Rect stackBounds, Rect movementBounds, Point displaySize, Rect stableInsets) {
        if (stackBounds.left <= movementBounds.centerX()) {
            ...
            stackBounds.offsetTo((displaySize.f59x - stableInsets.right) - this.mMinimizedVisibleSize, stackBounds.top);
        }
    }
    ...
    private float distanceToPoint(Point p, int x, int y) {
        return PointF.length(p.f59x - x, p.f60y - y);
    }
