        ...
        try {
            ...
            while (true) {
                ...
                if (element.equals(TAG_ARRAY)) {
                    parsingArray = true;
                    arrayList.clear();
                    arrayName = parser.getAttributeValue((String) null, "name");
    ...
    public void writeToProto(ProtoOutputStream proto) {
        ...
        for (int cluster = 0; cluster < this.mCpuClusters.length; cluster++) {
            ...
            for (Double speed : sPowerArrayMap.get(this.mCpuClusters[cluster].freqKey)) {
                proto.write((long) PowerProfileProto.CpuCluster.SPEED, speed.doubleValue());
            }
            for (Double corePower : sPowerArrayMap.get(this.mCpuClusters[cluster].corePowerKey)) {
                proto.write((long) PowerProfileProto.CpuCluster.CORE_POWER, corePower.doubleValue());
            }
            proto.end(token);
        }
