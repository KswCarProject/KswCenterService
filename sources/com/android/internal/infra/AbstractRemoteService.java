...
public abstract class AbstractRemoteService<S extends AbstractRemoteService<S, I>, I extends IInterface> implements IBinder.DeathRecipient {
    ...
    protected void scheduleBind() {
        if (this.mHandler.hasMessages(1)) {
            if (this.mVerbose) {
                Slog.m52v(this.mTag, "scheduleBind(): already scheduled");
                return;
            }
            return;
        }
    ...
    public void scheduleUnbind(boolean delay) {
        ...
        if (unbindDelay <= 0) {
            if (this.mVerbose) {
                ...
                Slog.m52v(str, "not scheduling unbind when value is " + unbindDelay);
                return;
            }
            return;
        }
        ...
        if (this.mVerbose) {
            ...
            Slog.m52v(str2, "unbinding in " + unbindDelay + "ms: " + this.mNextUnbind);
        }
    ...
    protected final void handlePendingRequest(BasePendingRequest<S, I> pendingRequest) {
        ...
        if (!handleIsBound()) {
            if (this.mVerbose) {
                String str = this.mTag;
                Slog.m52v(str, "handlePendingRequest(): queuing " + pendingRequest);
            }
        ...
        if (this.mVerbose) {
            String str2 = this.mTag;
            Slog.m52v(str2, "handlePendingRequest(): " + pendingRequest);
        }
    ...
    public void handleEnsureBound() {
        ...
        if (this.mVerbose) {
            Slog.m52v(this.mTag, "ensureBound()");
        }
        this.mBinding = true;
        ...
        if (!willBind) {
            String str = this.mTag;
            Slog.m50w(str, "could not bind to " + this.mIntent + " using flags " + flags);
            this.mBinding = false;
    ...
    private void handleEnsureUnbound() {
        if (handleIsBound() || this.mBinding) {
            if (this.mVerbose) {
                Slog.m52v(this.mTag, "ensureUnbound()");
            }
            this.mBinding = false;
            if (handleIsBound()) {
    ...
    private class RemoteServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            if (AbstractRemoteService.this.mVerbose) {
                Slog.m52v(AbstractRemoteService.this.mTag, "onServiceConnected()");
            }
            if (!AbstractRemoteService.this.mDestroyed && AbstractRemoteService.this.mBinding) {
                ...
                try {
                    ...
                    AbstractRemoteService.this.mService = (I) AbstractRemoteService.this.getServiceInterface(service);
        ...
        public void onServiceDisconnected(ComponentName name) {
            if (AbstractRemoteService.this.mVerbose) {
                Slog.m52v(AbstractRemoteService.this.mTag, "onServiceDisconnected()");
            }
        ...
        public void onBindingDied(ComponentName name) {
            if (AbstractRemoteService.this.mVerbose) {
                Slog.m52v(AbstractRemoteService.this.mTag, "onBindingDied()");
            }
    ...
    private boolean checkIfDestroyed() {
        if (this.mDestroyed && this.mVerbose) {
            String str = this.mTag;
            Slog.m52v(str, "Not handling operation as service for " + this.mComponentName + " is already destroyed");
        }
        return this.mDestroyed;
    }
        ...
        protected PendingRequest(final S service) {
            ...
            this.mTimeoutTrigger = new Runnable() { // from class: com.android.internal.infra.-$$Lambda$AbstractRemoteService$PendingRequest$IBoaBGXZQEXJr69u3aJF-LCJ42Y
                ...
                public final void run() {
                    AbstractRemoteService.PendingRequest.lambda$new$0(AbstractRemoteService.PendingRequest.this, service);
                }
            };
            this.mServiceHandler.postAtTime(this.mTimeoutTrigger, SystemClock.uptimeMillis() + service.getRemoteRequestMillis());
        }

        public static /* synthetic */ void lambda$new$0(PendingRequest pendingRequest, AbstractRemoteService service) {
            synchronized (pendingRequest.mLock) {
                if (pendingRequest.mCancelled) {
                    return;
                }
                pendingRequest.mCompleted = true;
                S remoteService = pendingRequest.mWeakService.get();
                if (remoteService != null) {
                    String str = pendingRequest.mTag;
                    Slog.m50w(str, "timed out after " + service.getRemoteRequestMillis() + " ms");
                    remoteService.finishRequest(pendingRequest);
                    pendingRequest.onTimeout(remoteService);
                    return;
                }
                Slog.m50w(pendingRequest.mTag, "timed out (no service)");
            }
        }
        ...
        public void run() {
            ...
            try {
                ...
                try {
                    ...
                    String str = TAG;
                    Slog.m50w(str, "exception handling async request (" + this + "): " + e);
                }
            } finally {
                finish();
            }
