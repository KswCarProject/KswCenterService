...
public final class NfcActivityManager extends IAppCallback.Stub implements Application.ActivityLifecycleCallbacks {
    ...
    class NfcApplicationState {
        ...
        public void unregister() {
            ...
            if (this.refCount == 0) {
                ...
                Log.m70e(NfcActivityManager.TAG, "-ve refcount for " + this.app);
            }
        }
    }
    ...
    void unregisterApplication(Application app) {
        NfcApplicationState appState = findAppState(app);
        if (appState == null) {
            Log.m70e(TAG, "app was not registered " + app);
            return;
        }
        appState.unregister();
    }
    ...
    public void setReaderMode(Binder token, int flags, Bundle extras) {
        if (DBG.booleanValue()) {
            Log.m72d(TAG, "Setting reader mode");
        }
        try {
    ...
    */
                            ...
                            if (uris != null) {
                                ...
                                while (i < length) {
                                    ...
                                    if (uri == null) {
                                        event = event2;
                                        try {
                                            Log.m70e(TAG, "Uri not allowed to be null.");
                                        } catch (Throwable th2) {
                                            th = th2;
                                            Binder.restoreCallingIdentity(ident);
                                            throw th;
                                        ...
                                        Log.m70e(TAG, "Uri needs to have either scheme file or scheme content");
                                    }
                                    i++;
                                    event2 = event;
                                }
    ...
    public void onActivityResumed(Activity activity) {
        synchronized (this) {
            ...
            if (DBG.booleanValue()) {
                Log.m72d(TAG, "onResume() for " + activity + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + state);
            }
            if (state == null) {
                return;
            }
    ...
    public void onActivityPaused(Activity activity) {
        synchronized (this) {
            NfcActivityState state = findActivityState(activity);
            if (DBG.booleanValue()) {
                Log.m72d(TAG, "onPause() for " + activity + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + state);
            }
            if (state == null) {
                return;
            }
    ...
    public void onActivityDestroyed(Activity activity) {
        synchronized (this) {
            NfcActivityState state = findActivityState(activity);
            if (DBG.booleanValue()) {
                Log.m72d(TAG, "onDestroy() for " + activity + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + state);
            }
            if (state != null) {
