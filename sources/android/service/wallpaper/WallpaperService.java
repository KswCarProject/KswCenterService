    ...
    public class Engine {
        ...
        public void notifyColorsChanged() {
            ...
            if (now - this.mLastColorInvalidation < 1000) {
                Log.m64w(WallpaperService.TAG, "This call has been deferred. You should only call notifyColorsChanged() once every 1.0 seconds.");
            ...
            try {
                ...
                if (this.mConnection != null) {
                    ...
                    Log.m64w(WallpaperService.TAG, "Can't notify system because wallpaper connection was not established.");
                }
            } catch (RemoteException e) {
                Log.m63w(WallpaperService.TAG, "Can't notify system because wallpaper connection was lost.", e);
            }
        }
        ...
        void updateSurface(boolean forceRelayout, boolean forceReport, boolean redrawNeeded) {
            ...
            if (this.mDestroyed) {
                Log.m64w(WallpaperService.TAG, "Ignoring updateSurface: destroyed");
            }
            ...
            try {
                ...
                this.mLayout.f2425x = 0;
                this.mLayout.f2426y = 0;
                if (fixedSize2) {
                ...
                if (this.mCreated) {
                    ...
                    try {
                        TypedArray windowStyle = WallpaperService.this.obtainStyledAttributes(C3132R.styleable.Window);
                        windowStyle.recycle();
                        ...
                        this.mLayout.windowAnimations = C3132R.C3136style.Animation_Wallpaper;
                                    ...
                                    try {
                                        ...
                                        try {
                                            if (this.mSession.addToDisplay(this.mWindow, this.mWindow.mSeq, this.mLayout, 0, this.mDisplay.getDisplayId(), this.mWinFrame, this.mContentInsets, this.mStableInsets, this.mOutsets, this.mDisplayCutout, this.mInputChannel, this.mInsetsState) < 0) {
                                                Log.m64w(WallpaperService.TAG, "Failed to add window while updating wallpaper surface.");
                                                return;
                                            } else {
                                                this.mCreated = true;
                        ...
                        try {
                            ...
                            try {
                                try {
                                    ...
                                    int w2 = this.mWinFrame.width();
                                    int h2 = this.mWinFrame.height();
                                    if (fixedSize2) {
                                        w = myWidth2;
                                        h = myHeight2;
                                    } else {
                                        Rect padding = this.mIWallpaperEngine.mDisplayPadding;
                                        w = w2 + padding.left + padding.right + this.mOutsets.left + this.mOutsets.right;
                                        h = h2 + padding.top + padding.bottom + this.mOutsets.top + this.mOutsets.bottom;
                                    ...
                                    try {
                                        ...
                                        try {
                                            this.mSurfaceHolder.setSurfaceFrameSize(w3, h3);
        ...
        void doCommand(WallpaperCommand cmd) {
            Bundle result;
            if (!this.mDestroyed) {
                result = onCommand(cmd.action, cmd.f252x, cmd.f253y, cmd.f254z, cmd.extras, cmd.sync);
            } else {
                result = null;
            }
            if (cmd.sync) {
        ...
        public void reportShown() {
            if (!this.mShownReported) {
                ...
                try {
                    ...
                    Log.m63w(WallpaperService.TAG, "Wallpaper host disappeared", e);
                }
            }
        }
        ...
        public void executeMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case 10050:
                    ...
                    return;
                default:
                    Log.m64w(WallpaperService.TAG, "Unknown message type " + message.what);
                    return;
            }
        }
    }
