    ...
    public final void adjustNotification(Adjustment adjustment) {
        if (isBound()) {
            try {
                ...
                Log.m65v(TAG, "Unable to contact notification manager", ex);
    ...
    private class NotificationAssistantServiceWrapper extends NotificationListenerService.NotificationListenerWrapper {
        ...
        public void onNotificationEnqueuedWithChannel(IStatusBarNotificationHolder sbnHolder, NotificationChannel channel) {
            try {
                ...
                Log.m63w(NotificationAssistantService.TAG, "onNotificationEnqueued: Error receiving StatusBarNotification", e);
            }
        }
        ...
        public void onNotificationSnoozedUntilContext(IStatusBarNotificationHolder sbnHolder, String snoozeCriterionId) {
            try {
                ...
                Log.m63w(NotificationAssistantService.TAG, "onNotificationSnoozed: Error receiving StatusBarNotification", e);
            }
        }
                ...
                case 1:
                    ...
                    if (adjustment != null) {
                        if (!NotificationAssistantService.this.isBound()) {
                            Log.m64w(NotificationAssistantService.TAG, "MSG_ON_NOTIFICATION_ENQUEUED: service not bound, skip.");
                            return;
                        }
                        try {
                            ...
                            Log.m65v(NotificationAssistantService.TAG, "Unable to contact notification manager", ex);
                            throw ex.rethrowFromSystemServer();
                        } catch (SecurityException e) {
                            Log.m63w(NotificationAssistantService.TAG, "Enqueue adjustment failed; no longer connected", e);
                            return;
                        }
                    }
                    return;
