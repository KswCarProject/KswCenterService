...
public final class ImageTransformation extends InternalTransformation implements Transformation, Parcelable {
    ...
    public void apply(ValueFinder finder, RemoteViews parentTemplate, int childViewId) throws Exception {
        ...
        if (Helper.sDebug) {
            Log.m72d(TAG, size + " multiple options on id " + childViewId + " to compare against");
        }
        for (int i = 0; i < size; i++) {
            Option option = this.mOptions.get(i);
            try {
                if (option.pattern.matcher(value).matches()) {
                    Log.m72d(TAG, "Found match at " + i + PluralRules.KEYWORD_RULE_SEPARATOR + option);
                ...
                Log.m64w(TAG, "Error matching regex #" + i + "(" + option.pattern + ") on id " + option.resId + PluralRules.KEYWORD_RULE_SEPARATOR + e.getClass());
                throw e;
            }
        }
        if (Helper.sDebug) {
            Log.m72d(TAG, "No match for " + value);
        }
    }
