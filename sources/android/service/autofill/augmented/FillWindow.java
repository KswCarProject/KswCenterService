...
public final class FillWindow implements AutoCloseable {
    ...
    public boolean update(PresentationParams.Area area, View rootView, long flags) {
        if (AugmentedAutofillService.sDebug) {
            String str = TAG;
            Log.m72d(str, "Updating " + area + " + with " + rootView);
        }
        ...
        if (smartSuggestion == null) {
            Log.m64w(TAG, "No SmartSuggestionParams");
            return false;
        }
        ...
        synchronized (this.mLock) {
            ...
            if (AugmentedAutofillService.sDebug) {
                String str2 = TAG;
                Log.m72d(str2, "Created FillWindow: params= " + smartSuggestion + " view=" + rootView);
            }
    ...
    public static /* synthetic */ boolean lambda$update$0(FillWindow fillWindow, View view, MotionEvent motionEvent) {
        if (motionEvent.getAction() == 4) {
            if (AugmentedAutofillService.sVerbose) {
                Log.m66v(TAG, "Outside touch detected, hiding the window");
            }
            fillWindow.hide();
            return false;
        }
        ...
        synchronized (this.mLock) {
            ...
            if (this.mProxy != null) {
                try {
                    ...
                    Log.m63w(TAG, "Error requesting to show fill window", e);
                }
        ...
        synchronized (this.mLock) {
            ...
            if (this.mProxy != null && this.mShowing) {
                try {
                    ...
                    Log.m63w(TAG, "Error requesting to hide fill window", e);
                }
            }
        }
    }
    ...
    public void destroy() {
        if (AugmentedAutofillService.sDebug) {
            String str = TAG;
            Log.m72d(str, "destroy(): mDestroyed=" + this.mDestroyed + " mShowing=" + this.mShowing + " mFillView=" + this.mFillView);
        }
        synchronized (this.mLock) {
            if (this.mDestroyed) {
                return;
    ...
    private final class FillWindowPresenter extends IAutofillWindowPresenter.Stub {
        ...
        public void show(WindowManager.LayoutParams p, Rect transitionEpicenter, boolean fitsSystemWindows, int layoutDirection) {
            if (AugmentedAutofillService.sDebug) {
                Log.m72d(FillWindow.TAG, "FillWindowPresenter.show()");
            }
        ...
        public void hide(Rect transitionEpicenter) {
            if (AugmentedAutofillService.sDebug) {
                Log.m72d(FillWindow.TAG, "FillWindowPresenter.hide()");
            }
