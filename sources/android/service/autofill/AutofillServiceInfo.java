    ...
    public AutofillServiceInfo(Context context, ServiceInfo si) {
        if (!Manifest.C0000permission.BIND_AUTOFILL_SERVICE.equals(si.permission)) {
            if (Manifest.C0000permission.BIND_AUTOFILL.equals(si.permission)) {
                Log.m64w(TAG, "AutofillService from '" + si.packageName + "' uses unsupported permission " + Manifest.C0000permission.BIND_AUTOFILL + ". It works for now, but might not be supported on future releases");
                ...
                Log.m64w(TAG, "AutofillService from '" + si.packageName + "' does not require permission " + Manifest.C0000permission.BIND_AUTOFILL_SERVICE);
        ...
        try {
            ...
            if (TAG_AUTOFILL_SERVICE.equals(parser.getName())) {
                ...
                TypedArray afsAttributes = resources.obtainAttributes(allAttributes, C3132R.styleable.AutofillService);
                ...
                Log.m70e(TAG, "Meta-data does not start with autofill-service tag");
            }
        } catch (PackageManager.NameNotFoundException | IOException | XmlPullParserException e) {
            Log.m69e(TAG, "Error parsing auto fill service meta-data", e);
        }
    ...
    private ArrayMap<String, Long> parseCompatibilityPackages(XmlPullParser parser, Resources resources) throws IOException, XmlPullParserException {
        ...
        while (true) {
            ...
            if (type == 1 || (type == 3 && parser.getDepth() <= outerDepth)) {
                ...
                try {
                    AttributeSet allAttributes = Xml.asAttributeSet(parser);
                    cpAttributes = resources.obtainAttributes(allAttributes, C3132R.styleable.AutofillService_CompatibilityPackage);
                    String name = cpAttributes.getString(0);
                    if (TextUtils.isEmpty(name)) {
                        Log.m70e(TAG, "Invalid compatibility package:" + name);
                    } else {
                        ...
                        if (maxVersionCodeStr != null) {
                            try {
                                ...
                                if (maxVersionCode.longValue() < 0) {
                                    Log.m70e(TAG, "Invalid compatibility max version code:" + maxVersionCode);
                                    XmlUtils.skipCurrentTag(parser);
                                }
                            } catch (NumberFormatException e) {
                                Log.m70e(TAG, "Invalid compatibility max version code:" + maxVersionCodeStr);
                                XmlUtils.skipCurrentTag(parser);
                            }
                        } else {
