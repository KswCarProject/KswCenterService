...
import android.p007os.PooledStringReader;
import android.p007os.PooledStringWriter;
...
public class AssistStructure implements Parcelable {
        ...
        protected boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            if (code == 2) {
                ...
                if (token != null) {
                    ...
                    Log.m64w(AssistStructure.TAG, "Caller supplied bad token type: " + token);
                    return true;
                }
    ...
    static final class ParcelTransferWriter extends Binder {
        ...
        void writeToParcel(AssistStructure as, Parcel out) {
            ...
            sb.append(" views");
            Log.m68i(AssistStructure.TAG, sb.toString());
        }
        ...
        private void fetchData() {
            ...
            try {
                ...
                try {
                    ...
                    Log.m63w(AssistStructure.TAG, "Failure reading AssistStructure data", e);
    ...
    public static class WindowNode {
        ...
        WindowNode(AssistStructure assist, ViewRootImpl root, boolean forAutoFill, int flags) {
            ...
            this.f22mX = rect.left - view.getLeft();
            this.f23mY = rect.top - view.getTop();
        ...
        WindowNode(ParcelTransferReader reader) {
            ...
            this.f22mX = in.readInt();
            this.f23mY = in.readInt();
        ...
        void writeSelfToParcel(Parcel out, PooledStringWriter pwriter, float[] tmpMatrix) {
            out.writeInt(this.f22mX);
            out.writeInt(this.f23mY);
    ...
    public static class ViewNode {
        ...
        ViewNode(ParcelTransferReader reader, int nestingLevel) {
            ...
            if ((67108864 & flags) != 0) {
                this.f20mX = in.readInt();
                this.f21mY = in.readInt();
                this.mWidth = in.readInt();
                this.mHeight = in.readInt();
            } else {
                int val = in.readInt();
                this.f20mX = val & 32767;
                this.f21mY = (val >> 16) & 32767;
                int val2 = in.readInt();
        ...
        */
        int writeSelfToParcel(Parcel out, PooledStringWriter pwriter, boolean sanitizeOnWrite, float[] tmpMatrix) {
            ...
            if ((this.f20mX & (-32768)) == 0 && (this.f21mY & (-32768)) == 0) {
            }
            flags |= 67108864;
            ...
            if ((flags & 67108864) == 0) {
                out.writeInt((this.f21mY << 16) | this.f20mX);
                out.writeInt((this.mHeight << 16) | this.mWidth);
            } else {
                out.writeInt(this.f20mX);
                out.writeInt(this.f21mY);
                out.writeInt(this.mWidth);
                out.writeInt(this.mHeight);
            }
        ...
        public void setWebDomain(String domain) {
            ...
            if (uri == null) {
                Log.m64w(AssistStructure.TAG, "Failed to parse web domain");
                return;
            }
    ...
    static class ViewNodeBuilder extends ViewStructure {
        ...
        public void setDimens(int left, int top, int scrollX, int scrollY, int width, int height) {
            this.mNode.f20mX = left;
            this.mNode.f21mY = top;
            ...
            public HtmlInfoNode createFromParcel(Parcel parcel) {
                ...
                if (names != null && values != null) {
                    if (names.length != values.length) {
                        Log.m64w(AssistStructure.TAG, "HtmlInfo attributes mismatch: names=" + names.length + ", values=" + values.length);
                    } else {
    ...
    public AssistStructure(Activity activity, boolean forAutoFill, int flags) {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            if (root.getView() == null) {
                Log.m64w(TAG, "Skipping window with dettached view: " + ((Object) root.getTitle()));
            } else {
    ...
    public void dump(boolean showSensitive) {
        ...
        if (this.mActivityComponent == null) {
            Log.m68i(TAG, "dump(): calling ensureData() first");
            ensureData();
        }
        Log.m68i(TAG, "Task id: " + this.mTaskId);
        StringBuilder sb = new StringBuilder();
        sb.append("Activity: ");
        if (this.mActivityComponent != null) {
        ...
        Log.m68i(TAG, "Sanitize on write: " + this.mSanitizeOnWrite);
        Log.m68i(TAG, "Flags: " + this.mFlags);
        int N = getWindowNodeCount();
        for (int i = 0; i < N; i++) {
            WindowNode node = getWindowNodeAt(i);
            Log.m68i(TAG, "Window #" + i + " [" + node.getLeft() + SmsManager.REGEX_PREFIX_DELIMITER + node.getTop() + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + node.getWidth() + "x" + node.getHeight() + "] " + ((Object) node.getTitle()));
    ...
    void dump(String prefix, ViewNode node, boolean showSensitive) {
        String safeText;
        Log.m68i(TAG, prefix + "View [" + node.getLeft() + SmsManager.REGEX_PREFIX_DELIMITER + node.getTop() + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + node.getWidth() + "x" + node.getHeight() + "] " + node.getClassName());
        int id = node.getId();
        if (id != 0) {
            StringBuilder sb = new StringBuilder();
            sb.append(prefix);
        ...
        if (scrollX != 0 || scrollY != 0) {
            Log.m68i(TAG, prefix + "  Scroll: " + scrollX + SmsManager.REGEX_PREFIX_DELIMITER + scrollY);
        }
        Matrix matrix = node.getTransformation();
        if (matrix != null) {
            Log.m68i(TAG, prefix + "  Transformation: " + matrix);
        }
        float elevation = node.getElevation();
        if (elevation != 0.0f) {
            Log.m68i(TAG, prefix + "  Elevation: " + elevation);
        }
        float alpha = node.getAlpha();
        if (alpha != 0.0f) {
            Log.m68i(TAG, prefix + "  Alpha: " + elevation);
        }
        CharSequence contentDescription = node.getContentDescription();
        if (contentDescription != null) {
            Log.m68i(TAG, prefix + "  Content description: " + ((Object) contentDescription));
        }
        ...
        if (text != null) {
            ...
            Log.m68i(TAG, prefix + "  Text (sel " + node.getTextSelectionStart() + NativeLibraryHelper.CLEAR_ABI_OVERRIDE + node.getTextSelectionEnd() + "): " + safeText);
            ...
            Log.m68i(TAG, sb2.toString());
            Log.m68i(TAG, prefix + "  Text color fg: #" + Integer.toHexString(node.getTextColor()) + ", bg: #" + Integer.toHexString(node.getTextBackgroundColor()));
            StringBuilder sb3 = new StringBuilder();
            sb3.append(prefix);
            sb3.append("  Input type: ");
            sb3.append(node.getInputType());
            Log.m68i(TAG, sb3.toString());
            Log.m68i(TAG, prefix + "  Resource id: " + node.getTextIdEntry());
        }
        String webDomain = node.getWebDomain();
        if (webDomain != null) {
            Log.m68i(TAG, prefix + "  Web domain: " + webDomain);
        }
        ViewStructure.HtmlInfo htmlInfo = node.getHtmlInfo();
        if (htmlInfo != null) {
            Log.m68i(TAG, prefix + "  HtmlInfo: tag=" + htmlInfo.getTag() + ", attr=" + htmlInfo.getAttributes());
        }
        LocaleList localeList = node.getLocaleList();
        if (localeList != null) {
            Log.m68i(TAG, prefix + "  LocaleList: " + localeList);
        }
        String hint = node.getHint();
        if (hint != null) {
            Log.m68i(TAG, prefix + "  Hint: " + hint);
        }
        Bundle extras = node.getExtras();
        if (extras != null) {
            Log.m68i(TAG, prefix + "  Extras: " + extras);
        }
        if (node.isAssistBlocked()) {
            Log.m68i(TAG, prefix + "  BLOCKED");
        }
        AutofillId autofillId = node.getAutofillId();
        if (autofillId == null) {
            Log.m68i(TAG, prefix + " NO autofill ID");
        } else {
            Log.m68i(TAG, prefix + "  Autofill info: id= " + autofillId + ", type=" + node.getAutofillType() + ", options=" + Arrays.toString(node.getAutofillOptions()) + ", hints=" + Arrays.toString(node.getAutofillHints()) + ", value=" + node.getAutofillValue() + ", sanitized=" + node.isSanitized() + ", important=" + node.getImportantForAutofill());
        }
        int NCHILDREN = node.getChildCount();
        if (NCHILDREN > 0) {
            Log.m68i(TAG, prefix + "  Children:");
    ...
    boolean waitForReady() {
        ...
        synchronized (this) {
            ...
            if (this.mPendingAsyncChildren.size() > 0) {
                Log.m64w(TAG, "Skipping assist structure, waiting too long for async children (have " + this.mPendingAsyncChildren.size() + " remaining");
                skipStructure = true;
            }
        }
