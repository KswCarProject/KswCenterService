    ...
    */
    static void validateAgainstPreviousFreezePeriod(List<FreezePeriod> periods, LocalDate prevPeriodStart, LocalDate prevPeriodEnd, LocalDate now) {
        ...
        if (prevPeriodStart.isAfter(now) || prevPeriodEnd.isAfter(now)) {
            Log.m64w(TAG, "Previous period (" + prevPeriodStart + SmsManager.REGEX_PREFIX_DELIMITER + prevPeriodEnd + ") is after current date " + now);
        }
        List<FreezePeriod> allPeriods = canonicalizePeriods(periods);
        ...
        String periodsDescription = "Prev: " + prevPeriodStart + SmsManager.REGEX_PREFIX_DELIMITER + prevPeriodEnd + "; cur: " + curOrNextFreezeDates.first + SmsManager.REGEX_PREFIX_DELIMITER + curOrNextFreezeDates.second;
