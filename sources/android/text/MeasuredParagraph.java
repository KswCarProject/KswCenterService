    ...
    public static MeasuredParagraph buildForMeasurement(TextPaint paint, CharSequence text, int start, int end, TextDirectionHeuristic textDir, MeasuredParagraph recycle) {
        ...
        if (mt.mSpanned == null) {
            ...
            while (true) {
                ...
                MetricAffectingSpan[] spans = (MetricAffectingSpan[]) mt.mSpanned.getSpans(spanStart2, spanEnd, MetricAffectingSpan.class);
                mt.applyMetricsAffectingSpan(paint, (MetricAffectingSpan[]) TextUtils.removeEmptySpans(spans, mt.mSpanned, MetricAffectingSpan.class), spanStart2, spanEnd, null);
                spanStart = spanEnd;
            }
        }
        return mt;
