...
import android.p007os.storage.IStorageManager;
import android.p007os.strictmode.CleartextNetworkViolation;
import android.p007os.strictmode.ContentUriWithoutPermissionViolation;
import android.p007os.strictmode.CredentialProtectedWhileLockedViolation;
import android.p007os.strictmode.CustomViolation;
import android.p007os.strictmode.DiskReadViolation;
import android.p007os.strictmode.DiskWriteViolation;
import android.p007os.strictmode.ExplicitGcViolation;
import android.p007os.strictmode.FileUriExposedViolation;
import android.p007os.strictmode.ImplicitDirectBootViolation;
import android.p007os.strictmode.InstanceCountViolation;
import android.p007os.strictmode.IntentReceiverLeakedViolation;
import android.p007os.strictmode.LeakedClosableViolation;
import android.p007os.strictmode.NetworkViolation;
import android.p007os.strictmode.NonSdkApiUsedViolation;
import android.p007os.strictmode.ResourceMismatchViolation;
import android.p007os.strictmode.ServiceConnectionLeakedViolation;
import android.p007os.strictmode.SqliteObjectLeakedViolation;
import android.p007os.strictmode.UnbufferedIoViolation;
import android.p007os.strictmode.UntaggedSocketViolation;
import android.p007os.strictmode.Violation;
import android.p007os.strictmode.WebViewMethodCalledOnWrongThreadViolation;
...
import com.android.internal.p016os.BackgroundThread;
...
public final class StrictMode {
    ...
    private static final ViolationLogger LOGCAT_LOGGER = new ViolationLogger() { // from class: android.os.-$$Lambda$StrictMode$1yH8AK0bTwVwZOb9x8HoiSBdzr0
        @Override // android.p007os.StrictMode.ViolationLogger
    ...
    private static final Span NO_OP_SPAN = new Span() { // from class: android.os.StrictMode.7
        @Override // android.p007os.StrictMode.Span
        public void finish() {
        }
    };
    ...
    static /* synthetic */ void lambda$static$0(ViolationInfo info) {
        ...
        Log.m72d(TAG, msg + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + info.getStackTrace());
    }
    ...
    private static class AndroidBlockGuardPolicy implements BlockGuard.Policy {
        ...
        void onThreadPolicyViolation(ViolationInfo info) {
            if (StrictMode.LOG_V) {
                Log.m72d(StrictMode.TAG, "onThreadPolicyViolation; penalty=" + info.mPenaltyMask);
            }
            ...
            if (penaltyMask2 != 0) {
                ...
                if (justDropBox) {
                    StrictMode.dropboxViolationAsync(penaltyMask2, info);
                } else {
                    StrictMode.handleApplicationStrictModeViolation(penaltyMask2, info);
                }
            }
            if (!info.penaltyEnabled(268435456)) {
                ...
                if (listener != null && executor != null) {
                    try {
                        ...
                        Log.m69e(StrictMode.TAG, "ThreadPolicy penaltyCallback failed", e);
                        return;
                    }
                }
                return;
    ...
    public static void dropboxViolationAsync(final int penaltyMask, final ViolationInfo info) {
        ...
        if (LOG_V) {
            Log.m72d(TAG, "Dropboxing async; in-flight=" + outstanding);
        }
    ...
    static /* synthetic */ void lambda$dropboxViolationAsync$2(int penaltyMask, ViolationInfo info) {
        ...
        if (LOG_V) {
            Log.m72d(TAG, "Dropbox complete; in-flight=" + outstandingInner);
        }
    }
    ...
    public static void handleApplicationStrictModeViolation(int penaltyMask, ViolationInfo info) {
        ...
        try {
            try {
                ...
                if (am == null) {
                    Log.m64w(TAG, "No activity manager; failed to Dropbox violation.");
                } else {
                    am.handleApplicationStrictModeViolation(RuntimeInit.getApplicationObject(), penaltyMask, info);
                }
            } catch (RemoteException e) {
                if (!(e instanceof DeadObjectException)) {
                    Log.m69e(TAG, "RemoteException handling StrictMode violation", e);
                }
            }
        } finally {
    ...
    public static void setVmPolicy(VmPolicy policy) {
        synchronized (StrictMode.class) {
            ...
            if (netd != null) {
                ...
                Log.m64w(TAG, "Dropping requested network policy due to missing service!");
            }
    ...
    public static void onVmPolicyViolation(final Violation violation, boolean forceDeath) {
        ...
        if (sVmPolicy.mListener != null && sVmPolicy.mCallbackExecutor != null) {
            ...
            try {
                ...
                Log.m69e(TAG, "VmPolicy penaltyCallback failed", e);
            }
        }
    }
    ...
    public static class Span {
        ...
        public void finish() {
            ...
            synchronized (state) {
                ...
                state.mActiveSize--;
                if (StrictMode.LOG_V) {
                    Log.m72d(StrictMode.TAG, "Span finished=" + this.mName + "; size=" + state.mActiveSize);
                }
    ...
    public static Span enterCriticalSpan(String name) {
        ...
        synchronized (state) {
            ...
            if (LOG_V) {
                Log.m72d(TAG, "Span enter=" + name + "; size=" + state.mActiveSize);
            }
        }
        return span;
    }
