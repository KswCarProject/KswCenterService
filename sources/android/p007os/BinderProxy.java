...
public final class BinderProxy implements IBinder {
    ...
    private static final class ProxyMap {
        ...
        void set(long key, BinderProxy value) {
            ...
            if (size >= this.mWarnBucketSize) {
                int totalSize = size();
                Log.m66v("Binder", "BinderProxy map growth! bucket size = " + size + " total = " + totalSize);
                ...
                if (Build.IS_DEBUGGABLE && totalSize >= 20000) {
                    ...
                    if (totalUnclearedSize >= 20000) {
                        ...
                        Log.m66v("Binder", "BinderProxy map has many cleared entries: " + (totalSize - totalUnclearedSize) + " of " + totalSize + " are cleared");
                    }
                }
            }
        }
        ...
        public void dumpProxyInterfaceCounts() {
            ...
            Log.m66v("Binder", "BinderProxy descriptor histogram (top " + Integer.toString(10) + "):");
            for (int i = 0; i < sorted.length; i++) {
                Log.m66v("Binder", " #" + (i + 1) + PluralRules.KEYWORD_RULE_SEPARATOR + sorted[i]);
            }
        }
        ...
        public void dumpPerUidProxyCounts() {
            ...
            Log.m72d("Binder", "Per Uid Binder Proxy Counts:");
            for (int i = 0; i < counts.size(); i++) {
                int uid = counts.keyAt(i);
                int binderCount = counts.valueAt(i);
                Log.m72d("Binder", "UID : " + uid + "  count = " + binderCount);
            }
        }
    }
    ...
    public boolean transact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        ...
        if (this.mWarnOnBlocking && (flags & 1) == 0) {
            ...
            Log.m63w("Binder", "Outgoing transactions from this process must be FLAG_ONEWAY", new Throwable());
        }
    ...
    private static void sendDeathNotice(IBinder.DeathRecipient recipient) {
        try {
            ...
            Log.m63w("BinderNative", "Uncaught exception from death notification", exc);
        }
    }
}
