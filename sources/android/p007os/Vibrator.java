...
public abstract class Vibrator {
    ...
    private void loadVibrationIntensities(Context context) {
        this.mDefaultHapticFeedbackIntensity = loadDefaultIntensity(context, C3132R.integer.config_defaultHapticFeedbackIntensity);
        this.mDefaultNotificationVibrationIntensity = loadDefaultIntensity(context, C3132R.integer.config_defaultNotificationVibrationIntensity);
        this.mDefaultRingVibrationIntensity = loadDefaultIntensity(context, C3132R.integer.config_defaultRingVibrationIntensity);
    }
    ...
    public void vibrate(long milliseconds, AudioAttributes attributes) {
        try {
            ...
            Log.m69e(TAG, "Failed to create VibrationEffect", iae);
        }
    }
    ...
    public void vibrate(long[] pattern, int repeat, AudioAttributes attributes) {
        if (repeat < -1 || repeat >= pattern.length) {
            Log.m70e(TAG, "vibrate called with repeat index out of bounds (pattern.length=" + pattern.length + ", index=" + repeat + ")");
