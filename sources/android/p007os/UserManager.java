...
public class UserManager {
    ...
    public static boolean supportsMultipleUsers() {
        return getMaxSupportedUsers() > 1 && SystemProperties.getBoolean("fw.show_multiuserui", Resources.getSystem().getBoolean(C3132R.bool.config_enableMultiUserUI));
    }
    ...
    public static boolean isGuestUserEphemeral() {
        return Resources.getSystem().getBoolean(C3132R.bool.config_guestUserEphemeral);
    }
    ...
    public UserInfo createUser(String name, int flags) {
        try {
            ...
            if (user != null && !user.isAdmin() && !user.isDemo()) {
                this.mService.setUserRestriction(DISALLOW_SMS, true, user.f30id);
                this.mService.setUserRestriction(DISALLOW_OUTGOING_CALLS, true, user.f30id);
            }
            return user;
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
    ...
    public UserInfo createGuest(Context context, String name) {
        try {
            ...
            if (guest != null) {
                Settings.Secure.putStringForUser(context.getContentResolver(), Settings.Secure.SKIP_FIRST_USE_HINTS, "1", guest.f30id);
            }
            return guest;
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
    ...
    public UserInfo createRestrictedProfile(String name) {
        try {
            ...
            if (user != null) {
                AccountManager.get(this.mContext).addSharedAccountsFromParentUser(parentUserHandle, UserHandle.m110of(user.f30id));
            }
            return user;
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
    ...
    public List<UserHandle> getUserProfiles() {
        ...
        for (int userId : userIds) {
            result.add(UserHandle.m110of(userId));
        }
        return result;
    }
    ...
    public UserHandle getProfileParent(UserHandle user) {
        ...
        return UserHandle.m110of(info.f30id);
    }
    ...
    public static int getMaxSupportedUsers() {
        if (Build.f137ID.startsWith("JVP")) {
            return 1;
        }
        if (!ActivityManager.isLowRamDeviceStatic() || (Resources.getSystem().getConfiguration().uiMode & 15) == 4) {
            return SystemProperties.getInt("fw.max_users", Resources.getSystem().getInteger(C3132R.integer.config_multiuserMaximumUsers));
        }
        return 1;
    }
    ...
    public static final class EnforcingUser implements Parcelable {
        ...
        public UserHandle getUserHandle() {
            return UserHandle.m110of(this.userId);
        }
