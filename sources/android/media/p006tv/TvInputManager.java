...
import android.media.p006tv.ITvInputClient;
import android.media.p006tv.ITvInputHardwareCallback;
import android.media.p006tv.ITvInputManagerCallback;
...
public final class TvInputManager {
    ...
    private final ITvInputClient mClient = new ITvInputClient.Stub() { // from class: android.media.tv.TvInputManager.1
        @Override // android.media.p006tv.ITvInputClient
        public void onSessionCreated(String inputId, IBinder token, InputChannel channel, int seq) {
            synchronized (TvInputManager.this.mSessionCallbackRecordMap) {
                ...
                if (record == null) {
                    Log.m70e(TvInputManager.TAG, "Callback not found for " + token);
                    return;
                }
        ...
        public void onSessionReleased(int seq) {
            synchronized (TvInputManager.this.mSessionCallbackRecordMap) {
                ...
                TvInputManager.this.mSessionCallbackRecordMap.delete(seq);
                if (record == null) {
                    Log.m70e(TvInputManager.TAG, "Callback not found for seq:" + seq);
                    return;
                }
    ...
    public int getInputState(String inputId) {
        ...
        synchronized (this.mLock) {
            ...
            if (state == null) {
                Log.m64w(TAG, "Unrecognized input ID: " + inputId);
                return 2;
            }
    ...
    public Hardware acquireTvInputHardware(int deviceId, TvInputInfo info, final HardwareCallback callback) {
        try {
            return new Hardware(this.mService.acquireTvInputHardware(deviceId, new ITvInputHardwareCallback.Stub() { // from class: android.media.tv.TvInputManager.3
                @Override // android.media.p006tv.ITvInputHardwareCallback
    ...
    public static final class Session {
        ...
        public void release() {
            if (this.mToken == null) {
                Log.m64w(TvInputManager.TAG, "The session has been already released");
                return;
            }
            try {
                ...
                try {
                    if (type == 0) {
                        if (trackId != null && !containsTrack(this.mAudioTracks, trackId)) {
                            Log.m64w(TvInputManager.TAG, "Invalid audio trackId: " + trackId);
                            return;
                        }
                    } else if (type == 1) {
                        if (trackId != null && !containsTrack(this.mVideoTracks, trackId)) {
                            Log.m64w(TvInputManager.TAG, "Invalid video trackId: " + trackId);
                            return;
                        }
                    } else if (type == 2) {
                        if (trackId != null && !containsTrack(this.mSubtitleTracks, trackId)) {
                            Log.m64w(TvInputManager.TAG, "Invalid subtitle trackId: " + trackId);
                            return;
                        }
                    } else {
        ...
        private int sendInputEventOnMainLooperLocked(PendingEvent p) {
            if (this.mChannel != null) {
                ...
                Log.m64w(TvInputManager.TAG, "Unable to send input event to session: " + this.mToken + " dropping:" + event);
                return 0;
            }
            return 0;
        }
        ...
        void finishedInputEvent(int seq, boolean handled, boolean timeout) {
            synchronized (this.mHandler) {
                ...
                if (timeout) {
                    Log.m64w(TvInputManager.TAG, "Timeout waiting for session to handle input event after 2500 ms: " + this.mToken);
                } else {
