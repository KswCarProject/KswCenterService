...
public class AudioPolicy {
    ...
    private AudioPolicy(AudioPolicyConfig config, Context context, Looper looper, AudioPolicyFocusListener fl, AudioPolicyStatusListener sl, boolean isFocusPolicy, boolean isTestFocusPolicy, AudioPolicyVolumeCallback vc, MediaProjection projection) {
        ...
        if (looper != null) {
            ...
            Log.m70e(TAG, "No event handler due to looper without a thread");
        }
    ...
    public int attachMixes(List<AudioMix> mixes) {
        ...
        synchronized (this.mLock) {
            ...
            try {
                status = service.addMixForPolicy(cfg, m111cb());
                if (status == 0) {
                    this.mConfig.add(zeMixes);
                }
            } catch (RemoteException e) {
                Log.m69e(TAG, "Dead object in attachMixes", e);
                return -1;
            }
        }
        return status;
    ...
    public int detachMixes(List<AudioMix> mixes) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                status = service.removeMixForPolicy(cfg, m111cb());
                if (status == 0) {
                    this.mConfig.remove(zeMixes);
                }
            } catch (RemoteException e) {
                Log.m69e(TAG, "Dead object in detachMixes", e);
                return -1;
            }
        }
        return status;
    ...
    public boolean setUidDeviceAffinity(int uid, List<AudioDeviceInfo> devices) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                int status = service.setUidDeviceAffinity(m111cb(), uid, deviceTypes, deviceAdresses);
                z = status == 0;
            } catch (RemoteException e) {
                Log.m69e(TAG, "Dead object in setUidDeviceAffinity", e);
                return false;
            }
        }
        return z;
    ...
    public boolean removeUidDeviceAffinity(int uid) {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                int status = service.removeUidDeviceAffinity(m111cb(), uid);
                z = status == 0;
            } catch (RemoteException e) {
                Log.m69e(TAG, "Dead object in removeUidDeviceAffinity", e);
                return false;
            }
        }
        return z;
    ...
    private boolean policyReadyToUse() {
        ...
        synchronized (this.mLock) {
            if (this.mStatus != 2) {
                Log.m70e(TAG, "Cannot use unregistered AudioPolicy");
                return false;
                ...
                try {
                    if (this.mProjection != null) {
                        if (this.mProjection.getProjection().canProjectAudio()) {
                            ...
                            if ((isLoopbackRenderPolicy() || !canProjectAudio) && !canModifyAudioRouting) {
                                Slog.m50w(TAG, "Cannot use AudioPolicy for pid " + Binder.getCallingPid() + " / uid " + Binder.getCallingUid() + ", needs MODIFY_AUDIO_ROUTING or MediaProjection that can project audio.");
                                return false;
                            }
                            return true;
                        }
                    ...
                    Log.m70e(TAG, "Failed to check if MediaProjection#canProjectAudio");
    ...
    private int checkCallingOrSelfPermission(String permission) {
        ...
        Slog.m52v(TAG, "Null context, checking permission via ActivityManager");
    ...
    public int setFocusDuckingBehavior(int behavior) throws IllegalArgumentException, IllegalStateException {
        ...
        synchronized (this.mLock) {
            ...
            IAudioService service = getService();
            try {
                status = service.setFocusPropertiesForPolicy(behavior, m111cb());
                if (status == 0) {
                    this.mConfig.mDuckingPolicy = behavior;
                }
            } catch (RemoteException e) {
                Log.m69e(TAG, "Dead object in setFocusPropertiesForPolicy for behavior", e);
                return -1;
            }
        }
        return status;
    ...
    public AudioRecord createAudioRecordSink(AudioMix mix) throws IllegalArgumentException {
        if (!policyReadyToUse()) {
            Log.m70e(TAG, "Cannot create AudioRecord sink for AudioMix");
            return null;
        }
    ...
    public AudioTrack createAudioTrackSource(AudioMix mix) throws IllegalArgumentException {
        if (!policyReadyToUse()) {
            Log.m70e(TAG, "Cannot create AudioTrack source for AudioMix");
            return null;
        }
    ...
    /* renamed from: cb */
    public IAudioPolicyCallback m111cb() {
            ...
            switch (msg.what) {
                ...
                case 4:
                    if (AudioPolicy.this.mFocusListener != null) {
                        ...
                        Log.m70e(AudioPolicy.TAG, "Invalid null focus listener for focus request event");
                        return;
                    }
                case 5:
                    if (AudioPolicy.this.mFocusListener != null) {
                        ...
                        Log.m70e(AudioPolicy.TAG, "Invalid null focus listener for focus abandon event");
                        return;
                    }
                case 6:
                    if (AudioPolicy.this.mVolCb != null) {
                        ...
                        Log.m70e(AudioPolicy.TAG, "Invalid null volume event");
                        return;
                    }
                default:
                    Log.m70e(AudioPolicy.TAG, "Unknown event " + msg.what);
                    return;
            }
        }
    }
