                    ...
                    synchronized (sound) {
                        try {
                            if (status != 0) {
                                sound.state = 0;
                                sound.f118id = 0;
                                Log.m70e(MediaActionSound.TAG, "OnLoadCompleteListener() error: " + status + " loading sound: " + sound.name);
                                return;
                            }
                            switch (sound.state) {
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    playSoundId = sound.f118id;
                                    sound.state = 3;
                                    break;
                                default:
                                    Log.m70e(MediaActionSound.TAG, "OnLoadCompleteListener() called in wrong state: " + sound.state + " for sound: " + sound.name);
                                    break;
                            }
                            if (playSoundId != 0) {
        ...
        synchronized (sound) {
            if (sound.state == 0) {
                if (loadSound(sound) <= 0) {
                    Log.m70e(TAG, "load() error loading sound: " + soundName);
                }
            } else {
                Log.m70e(TAG, "load() called in wrong state: " + sound + " for sound: " + soundName);
            }
        }
    }
            ...
            if (i != 3) {
                switch (i) {
                    case 0:
                        loadSound(sound);
                        if (loadSound(sound) <= 0) {
                            Log.m70e(TAG, "play() error loading sound: " + soundName);
                            break;
                        }
                    case 1:
                        sound.state = 2;
                        break;
                    default:
                        Log.m70e(TAG, "play() called in wrong state: " + sound.state + " for sound: " + soundName);
                        break;
                }
            } else {
                this.mSoundPool.play(sound.f118id, 1.0f, 1.0f, 0, 0, 1.0f);
            }
        }
    }
