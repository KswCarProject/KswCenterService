...
class AppCompatPopupWindow extends PopupWindow {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C2132R.styleable.PopupWindow, defStyleAttr, defStyleRes);
        if (a.hasValue(C2132R.styleable.PopupWindow_overlapAnchor)) {
            setSupportOverlapAnchor(a.getBoolean(C2132R.styleable.PopupWindow_overlapAnchor, false));
        }
        setBackgroundDrawable(a.getDrawable(C2132R.styleable.PopupWindow_android_popupBackground));
        int sdk = Build.VERSION.SDK_INT;
        if (defStyleRes != 0 && sdk < 11 && a.hasValue(C2132R.styleable.PopupWindow_android_popupAnimationStyle)) {
            setAnimationStyle(a.getResourceId(C2132R.styleable.PopupWindow_android_popupAnimationStyle, -1));
        }
        a.recycle();
    ...
    private static void wrapOnScrollChangedListener(final PopupWindow popup) {
        try {
            ...
            Log.m71d(TAG, "Exception while installing workaround OnScrollChangedListener", e);
        }
    }
