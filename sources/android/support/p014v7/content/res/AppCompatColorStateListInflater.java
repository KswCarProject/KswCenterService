package android.support.p014v7.content.res;
        ...
        while (true) {
            ...
            if (type != i && ((depth = parser.getDepth()) >= innerDepth2 || type != 3)) {
                if (type != 2 || depth > innerDepth2) {
                    ...
                    TypedArray a = obtainAttributes(r, theme, attrs, C2132R.styleable.ColorStateListItem);
                    int baseColor = a.getColor(C2132R.styleable.ColorStateListItem_android_color, Color.MAGENTA);
                    float alphaMod = 1.0f;
                    if (a.hasValue(C2132R.styleable.ColorStateListItem_android_alpha)) {
                        alphaMod = a.getFloat(C2132R.styleable.ColorStateListItem_android_alpha, 1.0f);
                    } else if (a.hasValue(C2132R.styleable.ColorStateListItem_alpha)) {
                        alphaMod = a.getFloat(C2132R.styleable.ColorStateListItem_alpha, 1.0f);
                    }
                    a.recycle();
                    ...
                    while (j2 < numAttrs) {
                        ...
                        if (stateResId != 16843173 && stateResId != 16843551 && stateResId != C2132R.attr.alpha) {
                            int j3 = j + 1;
