...
import android.support.p011v4.provider.FontRequest;
...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        TypedArray array = resources.obtainAttributes(attrs, C1899R.styleable.FontFamily);
        String authority = array.getString(C1899R.styleable.FontFamily_fontProviderAuthority);
        String providerPackage = array.getString(C1899R.styleable.FontFamily_fontProviderPackage);
        String query = array.getString(C1899R.styleable.FontFamily_fontProviderQuery);
        int certsId = array.getResourceId(C1899R.styleable.FontFamily_fontProviderCerts, 0);
        int strategy = array.getInteger(C1899R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int timeoutMs = array.getInteger(C1899R.styleable.FontFamily_fontProviderFetchTimeout, 500);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        AttributeSet attrs = Xml.asAttributeSet(parser);
        TypedArray array = resources.obtainAttributes(attrs, C1899R.styleable.FontFamilyFont);
        int weight = array.getInt(C1899R.styleable.FontFamilyFont_fontWeight, 400);
        boolean isItalic = 1 == array.getInt(C1899R.styleable.FontFamilyFont_fontStyle, 0);
        int resourceId = array.getResourceId(C1899R.styleable.FontFamilyFont_font, 0);
        String filename = array.getString(C1899R.styleable.FontFamilyFont_font);
        array.recycle();
        while (parser.next() != 3) {
            skip(parser);
        }
