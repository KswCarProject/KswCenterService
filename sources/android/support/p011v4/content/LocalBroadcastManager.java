...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                Log.m66v(TAG, "Resolving type " + type2 + " scheme " + scheme + " of intent " + intent);
            }
            ArrayList<ReceiverRecord> entries = this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    Log.m66v(TAG, "Action list: " + entries);
                }
                ...
                while (true) {
                    ...
                    if (i3 < i4) {
                        ...
                        if (debug) {
                            Log.m66v(TAG, "Matching against filter " + receiver.filter);
                        }
                        if (receiver.broadcasting) {
                            if (debug) {
                                Log.m66v(TAG, "  Filter's target already added");
                            }
                            type = type2;
                            i = i3;
                            ...
                            if (match >= 0) {
                                if (debug) {
                                    Log.m66v(TAG, "  Filter matched!  match=0x" + Integer.toHexString(match));
                                }
                                if (receivers == null) {
                                ...
                                Log.m66v(TAG, "  Filter did not match: " + reason);
                            }
                        }
                        receivers2 = receivers;
                        i2 = i + 1;
