...
import android.support.p011v4.app.INotificationSideChannel;
...
public final class NotificationManagerCompat {
    ...
    private static class SideChannelManager implements Handler.Callback, ServiceConnection {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                Log.m72d(NotificationManagerCompat.TAG, "Connected to service " + componentName);
            }
        ...
        public void onServiceDisconnected(ComponentName componentName) {
            if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                Log.m72d(NotificationManagerCompat.TAG, "Disconnected from service " + componentName);
            }
        ...
        private void updateListenerMap() {
            ...
            for (ResolveInfo resolveInfo : resolveInfos) {
                if (enabledPackages.contains(resolveInfo.serviceInfo.packageName)) {
                    ...
                    if (resolveInfo.serviceInfo.permission != null) {
                        Log.m64w(NotificationManagerCompat.TAG, "Permission present on component " + componentName + ", not adding listener record.");
                    } else {
            ...
            for (ComponentName componentName2 : enabledComponents) {
                if (!this.mRecordMap.containsKey(componentName2)) {
                    if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                        Log.m72d(NotificationManagerCompat.TAG, "Adding listener record for " + componentName2);
                    }
            ...
            while (it.hasNext()) {
                ...
                if (!enabledComponents.contains(entry.getKey())) {
                    if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                        Log.m72d(NotificationManagerCompat.TAG, "Removing listener record for " + entry.getKey());
                    }
        ...
        private boolean ensureServiceBound(ListenerRecord record) {
            ...
            if (record.bound) {
                ...
                Log.m64w(NotificationManagerCompat.TAG, "Unable to bind to listener " + record.componentName);
        ...
        private void scheduleListenerRetry(ListenerRecord record) {
            ...
            if (record.retryCount > 6) {
                Log.m64w(NotificationManagerCompat.TAG, "Giving up on delivering " + record.taskQueue.size() + " tasks to " + record.componentName + " after " + record.retryCount + " retries");
                record.taskQueue.clear();
                return;
            }
            int delayMs = (1 << (record.retryCount - 1)) * 1000;
            if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                Log.m72d(NotificationManagerCompat.TAG, "Scheduling retry for " + delayMs + " ms");
            }
        ...
        private void processListenerQueue(ListenerRecord record) {
            if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                Log.m72d(NotificationManagerCompat.TAG, "Processing component " + record.componentName + ", " + record.taskQueue.size() + " queued tasks");
            }
            ...
            while (true) {
                ...
                try {
                    if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                        Log.m72d(NotificationManagerCompat.TAG, "Sending task " + task);
                    }
                    task.send(record.service);
                    record.taskQueue.remove();
                } catch (DeadObjectException e) {
                    if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                        Log.m72d(NotificationManagerCompat.TAG, "Remote service has died: " + record.componentName);
                    }
                } catch (RemoteException e2) {
                    Log.m63w(NotificationManagerCompat.TAG, "RemoteException communicating with " + record.componentName, e2);
                }
            }
            if (!record.taskQueue.isEmpty()) {
                scheduleListenerRetry(record);
    ...
    private static class NotifyTask implements Task {
        ...
        @Override // android.support.p011v4.app.NotificationManagerCompat.Task
        public void send(INotificationSideChannel service) throws RemoteException {
            service.notify(this.packageName, this.f268id, this.tag, this.notif);
        }

        public String toString() {
            return "NotifyTask[packageName:" + this.packageName + ", id:" + this.f268id + ", tag:" + this.tag + "]";
        }
    }
    ...
    private static class CancelTask implements Task {
        ...
        public void send(INotificationSideChannel service) throws RemoteException {
            if (this.all) {
                ...
                service.cancel(this.packageName, this.f267id, this.tag);
            }
        }

        public String toString() {
            return "CancelTask[packageName:" + this.packageName + ", id:" + this.f267id + ", tag:" + this.tag + ", all:" + this.all + "]";
        }
    }
}
