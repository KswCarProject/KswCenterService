...
public final class BluetoothGattServer implements BluetoothProfile {
    ...
    private final IBluetoothGattServerCallback mBluetoothGattServerCallback = new IBluetoothGattServerCallback.Stub() { // from class: android.bluetooth.BluetoothGattServer.1
        ...
        public void onServerRegistered(int status, int serverIf) {
            Log.m72d(BluetoothGattServer.TAG, "onServerRegistered() - status=" + status + " serverIf=" + serverIf);
            synchronized (BluetoothGattServer.this.mServerIfLock) {
                if (BluetoothGattServer.this.mCallback != null) {
                    ...
                    Log.m70e(BluetoothGattServer.TAG, "onServerRegistered: mCallback is null");
                }
            }
        }
        ...
        public void onServerConnectionState(int status, int serverIf, boolean connected, String address) {
            Log.m72d(BluetoothGattServer.TAG, "onServerConnectionState() - status=" + status + " serverIf=" + serverIf + " device=" + address);
            try {
                ...
                Log.m63w(BluetoothGattServer.TAG, "Unhandled exception in callback", ex);
            }
        }

        @Override // android.bluetooth.IBluetoothGattServerCallback
        public void onServiceAdded(int status, BluetoothGattService service) {
            Log.m72d(BluetoothGattServer.TAG, "onServiceAdded() - handle=" + service.getInstanceId() + " uuid=" + service.getUuid() + " status=" + status);
        ...
        public void onCharacteristicReadRequest(String address, int transId, int offset, boolean isLong, int handle) {
            ...
            Log.m64w(BluetoothGattServer.TAG, "onCharacteristicReadRequest() no char for handle " + handle);
        }
        ...
        public void onDescriptorReadRequest(String address, int transId, int offset, boolean isLong, int handle) {
            ...
            Log.m64w(BluetoothGattServer.TAG, "onDescriptorReadRequest() no desc for handle " + handle);
        }
        ...
        public void onCharacteristicWriteRequest(String address, int transId, int offset, int length, boolean isPrep, boolean needRsp, int handle, byte[] value) {
            ...
            Log.m64w(BluetoothGattServer.TAG, "onCharacteristicWriteRequest() no char for handle " + handle);
        }
        ...
        public void onDescriptorWriteRequest(String address, int transId, int offset, int length, boolean isPrep, boolean needRsp, int handle, byte[] value) {
            ...
            Log.m64w(BluetoothGattServer.TAG, "onDescriptorWriteRequest() no desc for handle " + handle);
        }

        @Override // android.bluetooth.IBluetoothGattServerCallback
        public void onExecuteWrite(String address, int transId, boolean execWrite) {
            Log.m72d(BluetoothGattServer.TAG, "onExecuteWrite() - device=" + address + ", transId=" + transId + "execWrite=" + execWrite);
            BluetoothDevice device = BluetoothGattServer.this.mAdapter.getRemoteDevice(address);
            if (device == null) {
                return;
            }
        ...
        public void onNotificationSent(String address, int status) {
            ...
            try {
                ...
                Log.m64w(BluetoothGattServer.TAG, "Unhandled exception: " + ex);
            }
        }

        @Override // android.bluetooth.IBluetoothGattServerCallback
        public void onMtuChanged(String address, int mtu) {
            Log.m72d(BluetoothGattServer.TAG, "onMtuChanged() - device=" + address + ", mtu=" + mtu);
            BluetoothDevice device = BluetoothGattServer.this.mAdapter.getRemoteDevice(address);
            if (device == null) {
                return;
            }
        ...
        public void onPhyUpdate(String address, int txPhy, int rxPhy, int status) {
            Log.m72d(BluetoothGattServer.TAG, "onPhyUpdate() - device=" + address + ", txPHy=" + txPhy + ", rxPHy=" + rxPhy);
            BluetoothDevice device = BluetoothGattServer.this.mAdapter.getRemoteDevice(address);
            if (device == null) {
                return;
            }
        ...
        public void onConnectionUpdated(String address, int interval, int latency, int timeout, int status) {
            Log.m72d(BluetoothGattServer.TAG, "onConnectionUpdated() - Device=" + address + " interval=" + interval + " latency=" + latency + " timeout=" + timeout + " status=" + status);
            BluetoothDevice device = BluetoothGattServer.this.mAdapter.getRemoteDevice(address);
            if (device == null) {
                return;
            }
    ...
    boolean registerCallback(BluetoothGattServerCallback callback) {
        Log.m72d(TAG, "registerCallback()");
        if (this.mService == null) {
            Log.m70e(TAG, "GATT service not available");
            return false;
        }
        UUID uuid = UUID.randomUUID();
        Log.m72d(TAG, "registerCallback() - UUID=" + uuid);
        synchronized (this.mServerIfLock) {
            if (this.mCallback != null) {
                Log.m70e(TAG, "App can register callback only once");
                return false;
            }
    ...
    private void unregisterCallback() {
        Log.m72d(TAG, "unregisterCallback() - mServerIf=" + this.mServerIf);
    ...
    public boolean connect(BluetoothDevice device, boolean autoConnect) {
        Log.m72d(TAG, "connect() - device: " + device.getAddress() + ", auto: " + autoConnect);
        if (this.mService == null || this.mServerIf == 0) {
            return false;
        }
        try {
    ...
    public void cancelConnection(BluetoothDevice device) {
        Log.m72d(TAG, "cancelConnection() - device: " + device.getAddress());
        if (this.mService == null || this.mServerIf == 0) {
            return;
        }
        try {
    ...
    public boolean addService(BluetoothGattService service) {
        Log.m72d(TAG, "addService() - service: " + service.getUuid());
        if (this.mService == null || this.mServerIf == 0) {
            return false;
        }
    ...
    public boolean removeService(BluetoothGattService service) {
        ...
        Log.m72d(TAG, "removeService() - service: " + service.getUuid());
