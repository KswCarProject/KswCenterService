    ...
    public static boolean canConnectToPrivateDnsServer(String hostname) {
        ...
        try {
            ...
            if (!socket.isConnected()) {
                Log.m64w(TAG, String.format("Connection to %s failed.", hostname));
                if (socket != null) {
                    socket.close();
                }
                return false;
            ...
            Log.m64w(TAG, String.format("TLS handshake to %s succeeded.", hostname));
            if (socket != null) {
                socket.close();
            }
            return true;
        } catch (IOException e) {
            Log.m63w(TAG, String.format("TLS handshake to %s failed.", hostname), e);
            return false;
        }
    }
}
