...
public class DiscoverySession implements AutoCloseable {
    ...
    public void close() {
        ...
        if (mgr == null) {
            Log.m64w(TAG, "destroy: called post GC on WifiAwareManager");
            return;
        }
    ...
    public void setTerminated() {
        if (this.mTerminated) {
            Log.m64w(TAG, "terminate: already terminated.");
            return;
        }
        this.mTerminated = true;
        this.mMgr.clear();
    ...
    public void sendMessage(PeerHandle peerHandle, int messageId, byte[] message, int retryCount) {
        if (this.mTerminated) {
            Log.m64w(TAG, "sendMessage: called on terminated session");
            return;
        }
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m64w(TAG, "sendMessage: called post GC on WifiAwareManager");
        } else {
    ...
    public NetworkSpecifier createNetworkSpecifierOpen(PeerHandle peerHandle) {
        if (this.mTerminated) {
            Log.m64w(TAG, "createNetworkSpecifierOpen: called on terminated session");
            return null;
        }
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m64w(TAG, "createNetworkSpecifierOpen: called post GC on WifiAwareManager");
            return null;
        }
    ...
    public NetworkSpecifier createNetworkSpecifierPassphrase(PeerHandle peerHandle, String passphrase) {
        ...
        if (this.mTerminated) {
            Log.m64w(TAG, "createNetworkSpecifierPassphrase: called on terminated session");
            return null;
        }
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m64w(TAG, "createNetworkSpecifierPassphrase: called post GC on WifiAwareManager");
            return null;
        }
        int role = this instanceof SubscribeDiscoverySession ? 0 : 1;
    ...
    public NetworkSpecifier createNetworkSpecifierPmk(PeerHandle peerHandle, byte[] pmk) {
        ...
        if (this.mTerminated) {
            Log.m64w(TAG, "createNetworkSpecifierPmk: called on terminated session");
            return null;
        }
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m64w(TAG, "createNetworkSpecifierPmk: called post GC on WifiAwareManager");
            return null;
        }
        int role = this instanceof SubscribeDiscoverySession ? 0 : 1;
