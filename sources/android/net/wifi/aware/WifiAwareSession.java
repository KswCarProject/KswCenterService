...
public class WifiAwareSession implements AutoCloseable {
    ...
    public void publish(PublishConfig publishConfig, DiscoverySessionCallback callback, Handler handler) {
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m70e(TAG, "publish: called post GC on WifiAwareManager");
        } else if (this.mTerminated) {
            Log.m70e(TAG, "publish: called after termination");
        } else {
    ...
    public NetworkSpecifier createNetworkSpecifierOpen(int role, byte[] peer) {
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m70e(TAG, "createNetworkSpecifierOpen: called post GC on WifiAwareManager");
            return null;
        } else if (this.mTerminated) {
            Log.m70e(TAG, "createNetworkSpecifierOpen: called after termination");
            return null;
        } else {
    ...
    public NetworkSpecifier createNetworkSpecifierPassphrase(int role, byte[] peer, String passphrase) {
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m70e(TAG, "createNetworkSpecifierPassphrase: called post GC on WifiAwareManager");
            return null;
        } else if (this.mTerminated) {
            Log.m70e(TAG, "createNetworkSpecifierPassphrase: called after termination");
            return null;
    ...
    public NetworkSpecifier createNetworkSpecifierPmk(int role, byte[] peer, byte[] pmk) {
        WifiAwareManager mgr = this.mMgr.get();
        if (mgr == null) {
            Log.m70e(TAG, "createNetworkSpecifierPmk: called post GC on WifiAwareManager");
            return null;
        } else if (this.mTerminated) {
            Log.m70e(TAG, "createNetworkSpecifierPmk: called after termination");
            return null;
