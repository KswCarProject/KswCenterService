...
public abstract class InputEventReceiver {
    ...
    public final void finishInputEvent(InputEvent event, boolean handled) {
        ...
        if (this.mReceiverPtr == 0) {
            Log.m64w(TAG, "Attempted to finish an input event but the input event receiver has already been disposed.");
        } else {
            int index = this.mSeqMap.indexOfKey(event.getSequenceNumber());
            if (index < 0) {
                Log.m64w(TAG, "Attempted to finish an input event that is not in progress.");
            } else {
    ...
    public final boolean consumeBatchedInputEvents(long frameTimeNanos) {
        if (this.mReceiverPtr == 0) {
            Log.m64w(TAG, "Attempted to consume batched input events but the input event receiver has already been disposed.");
            return false;
        }
    ...
    private void dispatchMotionEventInfo(int motionEventType, int touchMoveNum) {
        try {
            ...
            Log.m70e(TAG, "cannot invoke setMotionEventInfo.");
        }
    }
}
