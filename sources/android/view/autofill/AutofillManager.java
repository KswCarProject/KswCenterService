...
public final class AutofillManager {
    ...
    public void enableCompatibilityMode() {
        synchronized (this.mLock) {
            if (Helper.sDebug) {
                Slog.m58d(TAG, "creating CompatibilityBridge for " + this.mContext);
            }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        synchronized (this.mLock) {
            ...
            if (isActiveLocked()) {
                Log.m64w(TAG, "New session was started before onCreate()");
                return;
            }
                ...
                if (client != null) {
                    ...
                    try {
                        ...
                        if (!sessionWasRestored) {
                            Log.m64w(TAG, "Session " + this.mSessionId + " could not be restored");
                            this.mSessionId = Integer.MAX_VALUE;
                            this.mState = 0;
                        } else {
                            if (Helper.sDebug) {
                                Log.m72d(TAG, "session " + this.mSessionId + " was restored");
                            }
                            client.autofillClientResetableStateAvailable();
                        }
                    } catch (RemoteException e) {
                        Log.m69e(TAG, "Could not figure out if there was an autofill session", e);
                    }
                }
            }
        }
    ...
    private boolean shouldIgnoreViewEnteredLocked(AutofillId id, int flags) {
        if (isDisabledByServiceLocked()) {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "ignoring notifyViewEntered(flags=" + flags + ", view=" + id + ") on state " + getStateAsStringLocked() + " because disabled by svc");
            }
            return true;
        } else if (isFinishedLocked() && (flags & 1) == 0 && this.mEnteredIds != null && this.mEnteredIds.contains(id)) {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "ignoring notifyViewEntered(flags=" + flags + ", view=" + id + ") on state " + getStateAsStringLocked() + " because view was already entered: " + this.mEnteredIds);
            }
            return true;
        } else {
            return false;
    ...
    private AutofillCallback notifyViewEnteredLocked(View view, int flags) {
        ...
        if (!this.mEnabled && !this.mEnabledForAugmentedAutofillOnly) {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "ignoring notifyViewEntered(" + id + "): disabled");
            }
            ...
            if (!isActiveLocked()) {
                ...
                if (this.mForAugmentedAutofillOnly && (flags & 1) != 0) {
                    if (Helper.sDebug) {
                        Log.m72d(TAG, "notifyViewEntered(" + id + "): resetting mForAugmentedAutofillOnly on manual request");
                    }
    ...
    private void notifyViewVisibilityChangedInternal(View view, int virtualId, boolean isVisible, boolean virtual) {
        synchronized (this.mLock) {
            if (this.mForAugmentedAutofillOnly) {
                if (Helper.sVerbose) {
                    Log.m66v(TAG, "notifyViewVisibilityChanged(): ignoring on augmented only mode");
                }
                return;
            }
            if (this.mEnabled && isActiveLocked()) {
                ...
                if (Helper.sVerbose) {
                    Log.m66v(TAG, "visibility changed for " + id + PluralRules.KEYWORD_RULE_SEPARATOR + isVisible);
                }
                if (!isVisible && this.mFillableIds != null && this.mFillableIds.contains(id)) {
                    if (Helper.sDebug) {
                        Log.m72d(TAG, "Hidding UI when view " + id + " became invisible");
                    }
                ...
                if (this.mTrackedViews != null) {
                    ...
                    Log.m66v(TAG, "Ignoring visibility change on " + id + ": no tracked views");
                }
            }
        }
    }
    ...
    public void notifyViewExited(View view, int virtualId) {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "notifyViewExited(" + view.getAutofillId() + ", " + virtualId);
        }
        if (!hasAutofillFeature()) {
            return;
        }
    ...
    public void notifyValueChanged(View view) {
        ...
        synchronized (this.mLock) {
            ...
            if (this.mForAugmentedAutofillOnly) {
                if (Helper.sVerbose) {
                    Log.m66v(TAG, "notifyValueChanged(): not notifying system server on augmented-only mode");
                }
                return;
            }
            if (this.mEnabled && isActiveLocked()) {
            ...
            if (Helper.sVerbose) {
                Log.m66v(TAG, "notifyValueChanged(" + view.getAutofillId() + "): ignoring on state " + getStateAsStringLocked());
            }
        }
    }
    ...
    public void notifyValueChanged(View view, int virtualId, AutofillValue value) {
        ...
        synchronized (this.mLock) {
            if (this.mForAugmentedAutofillOnly) {
                if (Helper.sVerbose) {
                    Log.m66v(TAG, "notifyValueChanged(): ignoring on augmented only mode");
                }
                return;
            }
            if (this.mEnabled && isActiveLocked()) {
            ...
            if (Helper.sVerbose) {
                Log.m66v(TAG, "notifyValueChanged(" + view.getAutofillId() + SettingsStringUtil.DELIMITER + virtualId + "): ignoring on state " + getStateAsStringLocked());
            }
        }
    }
    ...
    private void notifyViewClicked(AutofillId id) {
        ...
        if (Helper.sVerbose) {
            Log.m66v(TAG, "notifyViewClicked(): id=" + id + ", trigger=" + this.mSaveTriggerId);
        }
        synchronized (this.mLock) {
            if (this.mEnabled && isActiveLocked()) {
                if (this.mSaveTriggerId != null && this.mSaveTriggerId.equals(id)) {
                    if (Helper.sDebug) {
                        Log.m72d(TAG, "triggering commit by click of " + id);
                    }
                    commitLocked();
    ...
    public void onActivityFinishing() {
        ...
        synchronized (this.mLock) {
            if (this.mSaveOnFinish) {
                if (Helper.sDebug) {
                    Log.m72d(TAG, "onActivityFinishing(): calling commitLocked()");
                }
                commitLocked();
            } else {
                if (Helper.sDebug) {
                    Log.m72d(TAG, "onActivityFinishing(): calling cancelLocked()");
                }
                cancelLocked();
            }
        }
    ...
    public void commit() {
        ...
        if (Helper.sVerbose) {
            Log.m66v(TAG, "commit() called by app");
        }
        synchronized (this.mLock) {
            commitLocked();
        }
    ...
    public void cancel() {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "cancel() called by app");
        }
        if (!hasAutofillFeature()) {
            return;
        }
    ...
    public AutofillClient getClient() {
        ...
        if (client == null && Helper.sVerbose) {
            Log.m66v(TAG, "No AutofillClient for " + this.mContext.getPackageName() + " on context " + this.mContext);
        }
        return client;
    }
    ...
    public void onAuthenticationResult(int authenticationId, Intent data, View focusView) {
        ...
        if (Helper.sDebug) {
            Log.m72d(TAG, "onAuthenticationResult(): id= " + authenticationId + ", data=" + data);
        }
        synchronized (this.mLock) {
            if (isActiveLocked()) {
                ...
                try {
                    ...
                    Log.m69e(TAG, "Error delivering authentication result", e);
                }
            }
        }
    }
    ...
    public AutofillId getNextAutofillId() {
        ...
        if (id == null && Helper.sDebug) {
            Log.m72d(TAG, "getNextAutofillId(): client " + client + " returned null");
        }
        return id;
    }
    ...
    private void startSessionLocked(AutofillId id, Rect bounds, AutofillValue value, int flags) {
        int flags2;
        if ((this.mEnteredForAugmentedAutofillIds == null || !this.mEnteredForAugmentedAutofillIds.contains(id)) && !this.mEnabledForAugmentedAutofillOnly) {
            flags2 = flags;
        } else {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "Starting session for augmented autofill on " + id);
            }
            flags2 = flags | 8;
        }
        if (Helper.sVerbose) {
            Log.m66v(TAG, "startSessionLocked(): id=" + id + ", bounds=" + bounds + ", value=" + value + ", flags=" + flags2 + ", state=" + getStateAsStringLocked() + ", compatMode=" + isCompatibilityModeEnabledLocked() + ", augmentedOnly=" + this.mForAugmentedAutofillOnly + ", enabledAugmentedOnly=" + this.mEnabledForAugmentedAutofillOnly + ", enteredIds=" + this.mEnteredIds);
        }
        if (this.mForAugmentedAutofillOnly && !this.mEnabledForAugmentedAutofillOnly && (flags2 & 1) != 0) {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "resetting mForAugmentedAutofillOnly on manual autofill request");
            }
            this.mForAugmentedAutofillOnly = false;
        }
        if (this.mState != 0 && !isFinishedLocked() && (flags2 & 1) == 0) {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "not automatically starting session for " + id + " on state " + getStateAsStringLocked() + " and flags " + flags2);
                return;
            }
            return;
        }
        try {
            ...
            if ((extraFlags & 1) != 0) {
                if (Helper.sDebug) {
                    Log.m72d(TAG, "startSession(" + componentName + "): for augmented only");
                }
    ...
    public void finishSessionLocked() {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "finishSessionLocked(): " + getStateAsStringLocked());
        }
        if (isActiveLocked()) {
            try {
    ...
    private void cancelSessionLocked() {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "cancelSessionLocked(): " + getStateAsStringLocked());
        }
        if (isActiveLocked()) {
            try {
    ...
    private void updateSessionLocked(AutofillId id, Rect bounds, AutofillValue value, int action, int flags) {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "updateSessionLocked(): id=" + id + ", bounds=" + bounds + ", value=" + value + ", action=" + action + ", flags=" + flags);
        }
        try {
    ...
    private void ensureServiceClientAddedIfNeededLocked() {
        ...
        if (client != null && this.mServiceClient == null) {
            ...
            try {
                ...
                if (Helper.sVerbose) {
                    Log.m66v(TAG, "receiver results: flags=" + flags + " enabled=" + this.mEnabled + ", enabledForAugmentedOnly: " + this.mEnabledForAugmentedAutofillOnly);
                }
    ...
    public void setState(int flags) {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "setState(" + flags + PluralRules.KEYWORD_RULE_SEPARATOR + DebugUtils.flagsToString(AutofillManager.class, "SET_STATE_FLAG_", flags) + ")");
        }
        synchronized (this.mLock) {
            try {
                if ((flags & 32) != 0) {
    ...
    public void autofill(int sessionId, List<AutofillId> ids, List<AutofillValue> values) {
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (failedIds != null) {
                    if (Helper.sVerbose) {
                        Log.m66v(TAG, "autofill(): total failed views: " + failedIds);
                    }
                    try {
    ...
    private void setNotifyOnClickLocked(AutofillId id, boolean notify) {
        ...
        if (view == null) {
            Log.m64w(TAG, "setNotifyOnClick(): invalid id: " + id);
            return;
        }
    ...
    public void setSaveUiState(int sessionId, boolean shown) {
        if (Helper.sDebug) {
            Log.m72d(TAG, "setSaveUiState(" + sessionId + "): " + shown);
        }
        synchronized (this.mLock) {
            if (this.mSessionId != Integer.MAX_VALUE) {
                Log.m64w(TAG, "setSaveUiState(" + sessionId + ", " + shown + ") called on existing session " + this.mSessionId + "; cancelling it");
                cancelSessionLocked();
            }
            if (shown) {
    ...
    public void setSessionFinished(int newState, List<AutofillId> autofillableIds) {
        ...
        synchronized (this.mLock) {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "setSessionFinished(): from " + getStateAsStringLocked() + " to " + getStateAsString(newState) + "; autofillableIds=" + autofillableIds);
            }
            if (autofillableIds != null) {
    ...
    public void getAugmentedAutofillClient(IResultReceiver result) {
        synchronized (this.mLock) {
            ...
            try {
                ...
                Log.m64w(TAG, "Could not send AugmentedAutofillClient back: " + e);
            }
        }
    }
    ...
    public void requestHideFillUi(AutofillId id, boolean force) {
        ...
        if (Helper.sVerbose) {
            Log.m66v(TAG, "requestHideFillUi(" + id + "): anchor = " + anchor);
        }
        if (anchor == null) {
    ...
    public void notifyNoFillUi(int sessionId, AutofillId id, int sessionFinishedState) {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "notifyNoFillUi(): sessionId=" + sessionId + ", autofillId=" + id + ", sessionFinishedState=" + sessionFinishedState);
        }
        View anchor = findView(id);
        if (anchor == null) {
            return;
    ...
    public void onPendingSaveUi(int operation, IBinder token) {
        if (Helper.sVerbose) {
            Log.m66v(TAG, "onPendingSaveUi(" + operation + "): " + token);
        }
        synchronized (this.mLock) {
            try {
    ...
    public void post(Runnable runnable) {
        ...
        if (client == null) {
            if (Helper.sVerbose) {
                Log.m66v(TAG, "ignoring post() because client is null");
                return;
            }
            return;
        }
    ...
    private final class CompatibilityBridge implements AccessibilityManager.AccessibilityPolicy {
        ...
        private AccessibilityServiceInfo getCompatServiceInfo() {
            synchronized (AutofillManager.this.mLock) {
                ...
                try {
                    ...
                    Log.m70e(AutofillManager.TAG, "Cannot find compat autofill service:" + intent);
        ...
        public AccessibilityEvent onAccessibilityEvent(AccessibilityEvent event, boolean accessibilityEnabled, int relevantEventTypes) {
            ...
            if (Helper.sVerbose) {
                Log.m66v(AutofillManager.TAG, "onAccessibilityEvent(" + AccessibilityEvent.eventTypeToString(type) + "): virtualId=" + AccessibilityNodeInfo.getVirtualDescendantId(event.getSourceNodeId()) + ", client=" + AutofillManager.this.getClient());
            }
            if (type == 1) {
                synchronized (AutofillManager.this.mLock) {
    ...
    private class TrackedViews {
        ...
        TrackedViews(AutofillId[] trackedIds) {
            ...
            if (!ArrayUtils.isEmpty(trackedIds) && client != null) {
                if (client.autofillClientIsVisibleForAutofill()) {
                    if (Helper.sVerbose) {
                        Log.m66v(AutofillManager.TAG, "client is visible, check tracked ids");
                    }
            ...
            if (Helper.sVerbose) {
                Log.m66v(AutofillManager.TAG, "TrackedViews(trackedIds=" + Arrays.toString(trackedIds) + "):  mVisibleTrackedIds=" + this.mVisibleTrackedIds + " mInvisibleTrackedIds=" + this.mInvisibleTrackedIds);
            }
        ...
        void notifyViewVisibilityChangedLocked(AutofillId id, boolean isVisible) {
            if (Helper.sDebug) {
                Log.m72d(AutofillManager.TAG, "notifyViewVisibilityChangedLocked(): id=" + id + " isVisible=" + isVisible);
            }
            ...
            if (this.mVisibleTrackedIds == null) {
                if (Helper.sVerbose) {
                    Log.m66v(AutofillManager.TAG, "No more visible ids. Invisibile = " + this.mInvisibleTrackedIds);
                }
                AutofillManager.this.finishSessionLocked();
            }
        }
        ...
        void onVisibleForAutofillChangedLocked() {
            ...
            if (client != null) {
                if (Helper.sVerbose) {
                    Log.m66v(AutofillManager.TAG, "onVisibleForAutofillChangedLocked(): inv= " + this.mInvisibleTrackedIds + " vis=" + this.mVisibleTrackedIds);
                }
                if (this.mInvisibleTrackedIds != null) {
                    ...
                    for (int i = 0; i < numInvisibleTrackedIds; i++) {
                        ...
                        if (isVisible[i]) {
                            ...
                            if (Helper.sDebug) {
                                Log.m72d(AutofillManager.TAG, "onVisibleForAutofill() " + id + " became visible");
                            }
                        } else {
                ...
                if (this.mVisibleTrackedIds != null) {
                    ...
                    for (int i2 = 0; i2 < numVisibleTrackedIds; i2++) {
                        ...
                        if (isVisible2[i2]) {
                            ...
                            if (Helper.sDebug) {
                                Log.m72d(AutofillManager.TAG, "onVisibleForAutofill() " + id2 + " became invisible");
                            }
                        }
                    }
                }
            ...
            if (this.mVisibleTrackedIds == null) {
                if (Helper.sVerbose) {
                    Log.m66v(AutofillManager.TAG, "onVisibleForAutofillChangedLocked(): no more visible ids");
                }
                AutofillManager.this.finishSessionLocked();
            }
        }
    ...
    private static final class AutofillManagerClient extends IAutoFillManagerClient.Stub {
        ...
        static /* synthetic */ void lambda$startIntentSender$7(AutofillManager afm, IntentSender intentSender, Intent intent) {
            try {
                ...
                Log.m69e(AutofillManager.TAG, "startIntentSender() failed for intent:" + intentSender, e);
            }
        }
    ...
    private static final class AugmentedAutofillManagerClient extends IAugmentedAutofillManagerClient.Stub {
        ...
        public Rect getViewCoordinates(AutofillId id) {
            ...
            if (afm != null) {
                ...
                if (client == null) {
                    Log.m64w(AutofillManager.TAG, "getViewCoordinates(" + id + "): no autofill client");
                    return null;
                }
                View view = client.autofillClientFindViewByAutofillIdTraversal(id);
                if (view == null) {
                    Log.m64w(AutofillManager.TAG, "getViewCoordinates(" + id + "): could not find view");
                    return null;
                }
                ...
                if (Helper.sVerbose) {
                    Log.m66v(AutofillManager.TAG, "Coordinates for " + id + PluralRules.KEYWORD_RULE_SEPARATOR + rect);
                }
                return rect;
            }
            return null;
