    ...
    public Insets calculateInsets(Rect relativeFrame, boolean ignoreVisibility) {
        ...
        if (this.mTmpFrame.width() == relativeFrame.width()) {
            if (this.mTmpFrame.top == relativeFrame.top) {
                return Insets.m132of(0, this.mTmpFrame.height(), 0, 0);
            }
            return Insets.m132of(0, 0, 0, this.mTmpFrame.height());
        } else if (this.mTmpFrame.height() == relativeFrame.height()) {
            if (this.mTmpFrame.left == relativeFrame.left) {
                return Insets.m132of(this.mTmpFrame.width(), 0, 0, 0);
            }
            return Insets.m132of(0, 0, this.mTmpFrame.width(), 0);
        } else {
            return Insets.NONE;
        }
    }
