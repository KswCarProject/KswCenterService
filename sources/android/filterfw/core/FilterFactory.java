...
public class FilterFactory {
    ...
    public static void addFilterLibrary(String libraryPath) {
        if (mLogVerbose) {
            Log.m66v(TAG, "Adding filter library " + libraryPath);
        }
        synchronized (mClassLoaderGuard) {
            if (mLibraries.contains(libraryPath)) {
                if (mLogVerbose) {
                    Log.m66v(TAG, "Library already added");
                }
                return;
            }
    ...
    public void addPackage(String packageName) {
        if (mLogVerbose) {
            Log.m66v(TAG, "Adding package " + packageName);
        }
    ...
    public Filter createFilterByClassName(String className, String filterName) {
        if (mLogVerbose) {
            Log.m66v(TAG, "Looking up class " + className);
        }
        ...
        while (it.hasNext()) {
            ...
            try {
                if (mLogVerbose) {
                    Log.m66v(TAG, "Trying " + packageName + "." + className);
                }
                synchronized (mClassLoaderGuard) {
