...
public class SyncRunner extends GraphRunner {
    ...
    public SyncRunner(FilterContext context, FilterGraph graph, Class schedulerClass) {
        ...
        if (this.mLogVerbose) {
            Log.m66v(TAG, "Initializing SyncRunner");
        }
        ...
        try {
            ...
            if (this.mLogVerbose) {
                Log.m66v(TAG, "Setting up filters");
            }
            graph.setupFilters();
    ...
    public void run() {
        ...
        if (this.mDoneListener != null) {
            if (this.mLogVerbose) {
                Log.m66v(TAG, "Calling completion listener.");
            }
            this.mDoneListener.onRunnerDone(determinePostRunState());
        }
        if (this.mLogVerbose) {
            Log.m66v(TAG, "Run complete");
        }
    }
    ...
    protected void processFilterNode(Filter filter) {
        if (this.mLogVerbose) {
            Log.m66v(TAG, "Processing filter node");
        }
        ...
        if (filter.getStatus() == 6) {
            ...
            if (this.mLogVerbose) {
                Log.m66v(TAG, "Scheduling filter wakeup");
            }
    ...
    boolean performStep() {
        if (this.mLogVerbose) {
            Log.m66v(TAG, "Performing one step.");
        }
