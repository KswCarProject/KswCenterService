    ...
    */
    private static Animator createAnimatorFromXml(Resources res, Resources.Theme theme, XmlPullParser parser, AttributeSet attrs, AnimatorSet parent, int sequenceOrdering, float pixelSize) throws XmlPullParserException, IOException {
        ...
        while (true) {
            int depth2 = depth;
            int type = parser.next();
            if ((type != 3 || parser.getDepth() > depth2) && type != 1) {
                if (type == 2) {
                    ...
                    if (name.equals("objectAnimator")) {
                        ...
                        if (name.equals("set")) {
                            ...
                            if (theme == null) {
                                a = res.obtainAttributes(attrs, C3132R.styleable.AnimatorSet);
                            } else {
                                a = theme.obtainStyledAttributes(attrs, C3132R.styleable.AnimatorSet, 0, 0);
                            }
                            TypedArray a2 = a;
                            anim2.appendChangingConfigurations(a2.getChangingConfigurations());
                            int ordering = a2.getInt(0, 0);
        ...
        while (true) {
            ...
            if (type == 3 || type == 1) {
                ...
                if (name.equals("propertyValuesHolder")) {
                    if (theme != null) {
                        a = theme.obtainStyledAttributes(attrs, C3132R.styleable.PropertyValuesHolder, 0, 0);
                    } else {
                        a = res.obtainAttributes(attrs, C3132R.styleable.PropertyValuesHolder);
                    }
    ...
    private static int inferValueTypeOfKeyframe(Resources res, Resources.Theme theme, AttributeSet attrs) {
        ...
        int valueType = 0;
        if (theme != null) {
            a = theme.obtainStyledAttributes(attrs, C3132R.styleable.Keyframe, 0, 0);
        } else {
            a = res.obtainAttributes(attrs, C3132R.styleable.Keyframe);
        }
    ...
    private static ValueAnimator loadAnimator(Resources res, Resources.Theme theme, AttributeSet attrs, ValueAnimator anim, float pathErrorScale) throws Resources.NotFoundException {
        ...
        if (theme != null) {
            arrayAnimator = theme.obtainStyledAttributes(attrs, C3132R.styleable.Animator, 0, 0);
        } else {
            arrayAnimator = res.obtainAttributes(attrs, C3132R.styleable.Animator);
        }
        if (anim != null) {
            if (theme != null) {
                arrayObjectAnimator = theme.obtainStyledAttributes(attrs, C3132R.styleable.PropertyAnimator, 0, 0);
            } else {
                arrayObjectAnimator = res.obtainAttributes(attrs, C3132R.styleable.PropertyAnimator);
            }
