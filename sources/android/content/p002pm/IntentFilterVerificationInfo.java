...
public final class IntentFilterVerificationInfo implements Parcelable {
    ...
    public void setStatus(int s) {
        ...
        String str = TAG;
        Log.m64w(str, "Trying to set a non supported status: " + s);
    }
    ...
    String getStringFromXml(XmlPullParser parser, String attribute, String defaultValue) {
        ...
        if (value == null) {
            String msg = "Missing element under " + TAG + PluralRules.KEYWORD_RULE_SEPARATOR + attribute + " at " + parser.getPositionDescription();
            Log.m64w(TAG, msg);
            return defaultValue;
        }
        return value;
    ...
    public void readFromXml(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        if (this.mPackageName == null) {
            Log.m70e(TAG, "Package name cannot be null!");
        }
        int status = getIntFromXml(parser, "status", -1);
        if (status == -1) {
            String str = TAG;
            Log.m70e(str, "Unknown status value: " + status);
        }
                ...
                if (type != 3 || parser.getDepth() > outerDepth) {
                    if (type != 3 && type != 4) {
                        ...
                        if (tagName.equals(TAG_DOMAIN)) {
                            ...
                            String str2 = TAG;
                            Log.m64w(str2, "Unknown tag parsing IntentFilter: " + tagName);
                        }
