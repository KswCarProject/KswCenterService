...
public abstract class RegisteredServicesCache<V> {
    ...
    private UserServices<V> findOrCreateUserLocked(int userId, boolean loadFromFileIfNew) {
        ...
        if (services == null) {
            ...
            if (loadFromFileIfNew && this.mSerializerAndParser != null && (user = getUser(userId)) != null) {
                AtomicFile file = createFileForUser(user.f30id);
                if (file.getBaseFile().exists()) {
                    try {
                        try {
                            ...
                            Log.m63w(TAG, "Error reading persistent services for user " + user.f30id, e);
                        }
                    } finally {
    ...
    private void generateServicesMap(int[] changedUids, int userId) {
        ...
        for (ResolveInfo resolveInfo : resolveInfos) {
            try {
                ...
                if (info == null) {
                    Log.m64w(TAG, "Unable to load service info " + resolveInfo.toString());
                } else {
                    serviceInfos.add(info);
                }
            } catch (IOException | XmlPullParserException e) {
                Log.m63w(TAG, "Unable to load service info " + resolveInfo.toString(), e);
            }
        }
        synchronized (this.mServicesLock) {
            try {
            ...
            if (!marker.exists()) {
                ...
                try {
                    try {
                        ...
                        Log.m63w(TAG, "Error reading persistent services, starting from scratch", e);
                    }
                    try {
                        for (UserInfo user : getUsers()) {
                            UserServices<V> userServices = this.mUserServices.get(user.f30id);
                            if (userServices != null) {
                                writePersistentServicesLocked(userServices, user.f30id);
                            }
                        }
                        marker.createNewFile();
                    } catch (Exception e2) {
                        Log.m63w(TAG, "Migration failed", e2);
                    }
                    this.mUserServices.clear();
                } finally {
                    IoUtils.closeQuietly(is);
    ...
    private void writePersistentServicesLocked(UserServices<V> user, int userId) {
        ...
        try {
            ...
            Log.m63w(TAG, "Error writing accounts", e1);
            if (fos != null) {
