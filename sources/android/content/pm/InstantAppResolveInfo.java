...
public final class InstantAppResolveInfo implements Parcelable {
    ...
    public InstantAppResolveInfo(InstantAppDigest digest, String packageName, List<InstantAppIntentFilter> filters, int versionCode) {
        this(digest, packageName, filters, (long) versionCode, (Bundle) null);
    }
    ...
    public InstantAppResolveInfo(String hostName, String packageName, List<InstantAppIntentFilter> filters) {
        this(new InstantAppDigest(hostName), packageName, filters, -1, (Bundle) null);
    }

    public InstantAppResolveInfo(Bundle extras) {
        this(InstantAppDigest.UNDEFINED, (String) null, (List<InstantAppIntentFilter>) null, -1, extras, true);
    }
    ...
    InstantAppResolveInfo(Parcel in) {
        ...
        this.mDigest = (InstantAppDigest) in.readParcelable((ClassLoader) null);
        ...
        public InstantAppDigest(String hostName, int maxDigests) {
            if (hostName != null) {
                ...
                for (int i = 0; i < this.mDigestBytes.length; i++) {
                    this.mDigestPrefix[i] = (((this.mDigestBytes[i][0] & 255) << 24) | ((this.mDigestBytes[i][1] & 255) << WifiScanner.PnoSettings.PnoNetwork.FLAG_SAME_NETWORK) | ((this.mDigestBytes[i][2] & 255) << 8) | ((this.mDigestBytes[i][3] & 255) << 0)) & DIGEST_MASK;
                }
                return;
            }
            throw new IllegalArgumentException();
