...
class PlaybackSynthesisCallback extends AbstractSynthesisCallback {
    ...
    void stop() {
        synchronized (this.mStateLock) {
            ...
            if (this.mStatusCode == -2) {
                Log.m64w(TAG, "stop() called twice");
                return;
            }
    ...
    public int start(int sampleRateInHz, int audioFormat, int channelCount) {
        if (audioFormat != 3 && audioFormat != 2 && audioFormat != 4) {
            Log.m64w(TAG, "Audio format encoding " + audioFormat + " not supported. Please use one of AudioFormat.ENCODING_PCM_8BIT, AudioFormat.ENCODING_PCM_16BIT or AudioFormat.ENCODING_PCM_FLOAT");
        }
        ...
        synchronized (this.mStateLock) {
            try {
                if (channelConfig == 0) {
                    Log.m70e(TAG, "Unsupported number of channels :" + channelCount);
                    this.mStatusCode = -5;
                    return -1;
    ...
    public int done() {
        synchronized (this.mStateLock) {
            if (this.mDone) {
                ...
                this.mDone = true;
                if (this.mItem == null) {
                    Log.m64w(TAG, "done() was called before start() call");
