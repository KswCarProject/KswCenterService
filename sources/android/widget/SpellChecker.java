...
public class SpellChecker implements SpellCheckerSession.SpellCheckerSessionListener {
    ...
    public void resetSession() {
        ...
        if (this.mCurrentLocale == null || this.mTextServicesManager == null || this.mTextView.length() == 0 || !this.mTextServicesManager.isSpellCheckerEnabled() || this.mTextServicesManager.getCurrentSpellCheckerSubtype(true) == null) {
            ...
            this.mSpellCheckerSession = this.mTextServicesManager.newSpellCheckerSession((Bundle) null, this.mCurrentLocale, this, false);
    ...
    private int nextSpellCheckSpanIndex() {
        ...
        this.mSpellCheckSpans = (SpellCheckSpan[]) GrowingArrayUtils.append((T[]) this.mSpellCheckSpans, this.mLength, new SpellCheckSpan());
        this.mLength++;
    ...
    public void spellCheck(int start, int end) {
        ...
        if (locale == null || this.mCurrentLocale == null || !this.mCurrentLocale.equals(locale)) {
            ...
            if (isSessionActive != (this.mTextServicesManager != null && this.mTextServicesManager.isSpellCheckerEnabled())) {
                resetSession();
            }
        }
        if (isSessionActive) {
        ...
        while (true) {
            ...
            if (sequenceNumber == this.mIds[k2]) {
                ...
                if (isInDictionary || !looksLikeTypo) {
                    ...
                    int spellCheckSpanStart = editable.getSpanStart(spellCheckSpan2);
                    int spellCheckSpanEnd = editable.getSpanEnd(spellCheckSpan2);
                    if (i == -1 || i2 == -1) {
                    ...
                    if (spellCheckSpanStart < 0 || spellCheckSpanEnd <= spellCheckSpanStart || end <= start || (tempSuggestionSpan = this.mSuggestionSpanCache.get(key)) == null) {
                        return spellCheckSpan2;
                    }
                    editable.removeSpan(tempSuggestionSpan);
                    int i3 = spellCheckSpanStart;
                    this.mSuggestionSpanCache.remove((key = Long.valueOf(TextUtils.packRangeInLong(start, end))));
                    return spellCheckSpan2;
                }
                SpellCheckSpan spellCheckSpan3 = spellCheckSpan;
                createMisspelledSuggestionSpan(editable, suggestionsInfo, spellCheckSpan, offset, length);
                return spellCheckSpan3;
            }
            k = k2 + 1;
        }
    }
