    ...
    private class SpellParser {
        ...
        public void parse(int start, int end) {
            ...
            if (end > max) {
                ...
                Log.m64w(str, "Parse invalid region, from " + start + " to " + end);
                parseEnd = max;
            } else {
                parseEnd = end;
            }
            ...
            if (SpellChecker.this.mIsSentenceSpellCheckSupported) {
                ...
                if (spellCheckEnd >= start) {
                    if (spellCheckEnd <= spellCheckStart) {
                        String str = SpellChecker.TAG;
                        Log.m64w(str, "Trying to spellcheck invalid region, from " + start + " to " + end);
                                ...
                                if (wordEnd == start) {
                                    ...
                                    while (true) {
                                        if (i5 >= spellCheckSpans.length) {
                                            break;
                                        } else if (editable.getSpanEnd(spellCheckSpans[i5]) != start) {
                                            i5++;
                                        } else {
