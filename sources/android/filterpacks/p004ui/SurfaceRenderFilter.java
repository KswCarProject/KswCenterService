...
public class SurfaceRenderFilter extends Filter implements SurfaceHolder.Callback {
    ...
    public void process(FilterContext context) {
        Frame gpuFrame;
        if (!this.mIsBound) {
            Log.m64w(TAG, this + ": Ignoring frame as there is no surface to render to!");
            return;
        }
        if (this.mLogVerbose) {
            Log.m66v(TAG, "Starting frame processing");
        }
        ...
        if (currentAspectRatio != this.mAspectRatio) {
            if (this.mLogVerbose) {
                Log.m66v(TAG, "New aspect ratio: " + currentAspectRatio + ", previously: " + this.mAspectRatio);
            }
            this.mAspectRatio = currentAspectRatio;
            updateTargetRect();
        }
        if (this.mLogVerbose) {
            Log.m66v(TAG, "Got input format: " + input.getFormat());
        }
