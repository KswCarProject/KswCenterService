...
public class KeyboardView extends View implements View.OnClickListener {
    ...
    public KeyboardView(Context context, AttributeSet attrs) {
        this(context, attrs, C3132R.attr.keyboardViewStyle);
    }
    ...
    public KeyboardView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mBackgroundDimAmount = this.mContext.obtainStyledAttributes(C3132R.styleable.Theme).getFloat(2, 0.5f);
        ...
        this.mDisambiguateSwipe = getResources().getBoolean(C3132R.bool.config_swipeDisambiguation);
    ...
    private void onBufferDraw() {
        ...
        if (invalidKey2 != null && canvas.getClipBounds(clipRegion) && (invalidKey2.f110x + kbdPaddingLeft) - 1 <= clipRegion.left && (invalidKey2.f111y + kbdPaddingTop) - 1 <= clipRegion.top && invalidKey2.f110x + invalidKey2.width + kbdPaddingLeft + 1 >= clipRegion.right && invalidKey2.f111y + invalidKey2.height + kbdPaddingTop + 1 >= clipRegion.bottom) {
            drawSingleKey = true;
        }
        ...
        while (i < keyCount2) {
            ...
            if (drawSingleKey2 && invalidKey2 != key) {
                ...
                canvas.translate(key.f110x + kbdPaddingLeft, key.f111y + kbdPaddingTop);
    ...
    /* JADX WARN: Code restructure failed: missing block: B:24:0x0065, code lost:
        r16 = r4;
        r17 = r6;
        java.lang.System.arraycopy(r18.mDistances, r15, r18.mDistances, r15 + r10, (r18.mDistances.length - r15) - r10);
    ...
    */
    private int getKeyIndices(int x, int y, int[] allKeys) {
        ...
        int primaryIndex = -1;
        int primaryIndex2 = 0;
        while (primaryIndex2 < keyCount) {
            Keyboard.Key key = keys[nearestKeyIndices[primaryIndex2]];
            int dist = 0;
            boolean isInside = key.isInside(i, i2);
            if (isInside) {
                primaryIndex = nearestKeyIndices[primaryIndex2];
            }
        ...
        if (primaryIndex == -1) {
            int primaryIndex3 = closestKey;
            return primaryIndex3;
        }
        return primaryIndex;
    }
    ...
    public void showKey(int keyIndex) {
        ...
        if (!this.mPreviewCentered) {
            this.mPopupPreviewX = (key.f110x - this.mPreviewText.getPaddingLeft()) + this.mPaddingLeft;
            this.mPopupPreviewY = (key.f111y - popupHeight) + this.mPreviewOffset;
        } else {
        ...
        if (this.mPopupPreviewY + this.mCoordinates[1] < 0) {
            if (key.f110x + key.width <= getWidth() / 2) {
    ...
    private void sendAccessibilityEventForUnicodeCharacter(int eventType, int code) {
        ...
        if (this.mAccessibilityManager.isEnabled()) {
            ...
            if (code != 10) {
                switch (code) {
                    case -6:
                        text = this.mContext.getString(C3132R.string.keyboardview_keycode_alt);
                        break;
                    case -5:
                        text = this.mContext.getString(C3132R.string.keyboardview_keycode_delete);
                        break;
                    case -4:
                        text = this.mContext.getString(C3132R.string.keyboardview_keycode_done);
                        break;
                    case -3:
                        text = this.mContext.getString(C3132R.string.keyboardview_keycode_cancel);
                        break;
                    case -2:
                        text = this.mContext.getString(C3132R.string.keyboardview_keycode_mode_change);
                        break;
                    case -1:
                        text = this.mContext.getString(C3132R.string.keyboardview_keycode_shift);
                        break;
                    default:
                ...
                text = this.mContext.getString(C3132R.string.keyboardview_keycode_enter);
            }
    ...
    public void invalidateKey(int keyIndex) {
        ...
        this.mDirtyRect.union(key.f110x + this.mPaddingLeft, key.f111y + this.mPaddingTop, key.f110x + key.width + this.mPaddingLeft, key.f111y + key.height + this.mPaddingTop);
        onBufferDraw();
        invalidate(key.f110x + this.mPaddingLeft, key.f111y + this.mPaddingTop, key.f110x + key.width + this.mPaddingLeft, key.f111y + key.height + this.mPaddingTop);
    }
    ...
    protected boolean onLongPress(Keyboard.Key popupKey) {
        ...
        getLocationInWindow(this.mCoordinates);
        this.mPopupX = popupKey.f110x + this.mPaddingLeft;
        this.mPopupY = popupKey.f111y + this.mPaddingTop;
    ...
    public boolean repeatKey() {
        ...
        detectAndSendKey(this.mCurrentKey, key.f110x, key.f111y, this.mLastTapTime);
        return true;
    }
