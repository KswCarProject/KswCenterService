...
public final class ScriptGroup extends BaseObj {
    ...
    /* loaded from: classes3.dex */
    static class C1687IO {
        Allocation mAllocation;
        Script.KernelID mKID;

        C1687IO(Script.KernelID s) {
            this.mKID = s;
        }
    }
    ...
    public Object[] execute(Object... inputs) {
        if (inputs.length < this.mInputs2.size()) {
            Log.m70e(TAG, toString() + " receives " + inputs.length + " inputs, less than expected " + this.mInputs2.size());
            return null;
        }
        if (inputs.length > this.mInputs2.size()) {
            Log.m68i(TAG, toString() + " receives " + inputs.length + " inputs, more than expected " + this.mInputs2.size());
        }
        int i = 0;
        for (int i2 = 0; i2 < this.mInputs2.size(); i2++) {
            Object obj = inputs[i2];
            if ((obj instanceof Future) || (obj instanceof Input)) {
                Log.m70e(TAG, toString() + ": input " + i2 + " is a future or unbound value");
                return null;
            }
    ...
    public static final class Builder {
        ...
        public ScriptGroup create() {
            ...
            validateDAG();
            ArrayList<C1687IO> inputs = new ArrayList<>();
            ArrayList<C1687IO> outputs = new ArrayList<>();
            ...
            for (int idx2 = 0; idx2 < this.mNodes.size(); idx2++) {
                ...
                while (ct2 < n.mKernels.size()) {
                    ...
                    if (!hasInput) {
                        inputs.add(new C1687IO(kid));
                    }
                    if (!hasOutput) {
                        outputs.add(new C1687IO(kid));
                    }
                    ct2++;
                    idx = idx3;
                }
