...
abstract class AndroidKeyStoreSignatureSpiBase extends SignatureSpi implements KeyStoreCryptoOperation {
    ...
    private void ensureKeystoreOperationInitialized() throws InvalidKeyException {
        ...
        if (this.mKey != null) {
            ...
            OperationResult opResult = this.mKeyStore.begin(this.mKey.getAlias(), this.mSigning ? 2 : 3, true, keymasterInputArgs, (byte[]) null, this.mKey.getUid());
            if (opResult != null) {
    ...
    public final byte[] engineSign() throws SignatureException {
        if (this.mCachedException == null) {
            try {
                ensureKeystoreOperationInitialized();
                byte[] signature = this.mMessageStreamer.doFinal(EmptyArray.BYTE, 0, 0, (byte[]) null, KeyStoreCryptoOperationUtils.getRandomBytesToMixIntoKeystoreRng(this.appRandom, getAdditionalEntropyAmountForSign()));
    ...
    public final boolean engineVerify(byte[] signature) throws SignatureException {
        ...
        if (this.mCachedException == null) {
            try {
                ensureKeystoreOperationInitialized();
                try {
                    byte[] output = this.mMessageStreamer.doFinal(EmptyArray.BYTE, 0, 0, signature, (byte[]) null);
                    if (output.length == 0) {
                        verified = true;
                        resetWhilePreservingInitState();
                        return verified;
