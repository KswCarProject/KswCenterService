...
public class WebViewLibraryLoader {
        ...
        public static void main(String[] args) {
            ...
            try {
                if (args.length == 2 && args[0] != null && args[1] != null) {
                    ...
                    Log.m66v(str, "RelroFileCreator (64bit = " + is64Bit + "), package: " + packageName + " library: " + libraryFileName);
                    if (!WebViewLibraryLoader.sAddressSpaceReserved) {
                        Log.m70e(WebViewLibraryLoader.LOGTAG, "can't create relro file; address space not reserved");
                        return;
                    }
                    ...
                    try {
                        ...
                        Log.m69e(WebViewLibraryLoader.LOGTAG, "error notifying update service", e);
                    }
                    if (!result) {
                        Log.m70e(WebViewLibraryLoader.LOGTAG, "failed to create relro file");
                    }
                    System.exit(0);
                    return;
                }
                String str2 = WebViewLibraryLoader.LOGTAG;
                Log.m70e(str2, "Invalid RelroFileCreator args: " + Arrays.toString(args));
                try {
                    WebViewFactory.getUpdateServiceUnchecked().notifyRelroCreationCompleted();
                } catch (RemoteException e2) {
                    Log.m69e(WebViewLibraryLoader.LOGTAG, "error notifying update service", e2);
                }
                if (0 == 0) {
                    Log.m70e(WebViewLibraryLoader.LOGTAG, "failed to create relro file");
                }
                ...
                try {
                    ...
                    Log.m69e(WebViewLibraryLoader.LOGTAG, "error notifying update service", e3);
                }
                if (0 == 0) {
                    Log.m70e(WebViewLibraryLoader.LOGTAG, "failed to create relro file");
                }
    ...
    static void createRelroFile(boolean is64Bit, String packageName, String libraryFileName) {
        ...
        Runnable crashHandler = new Runnable() { // from class: android.webkit.WebViewLibraryLoader.1
            ...
            public void run() {
                try {
                    String str = WebViewLibraryLoader.LOGTAG;
                    Log.m70e(str, "relro file creator for " + abi + " crashed. Proceeding without");
                    WebViewFactory.getUpdateService().notifyRelroCreationCompleted();
                } catch (RemoteException e) {
                    String str2 = WebViewLibraryLoader.LOGTAG;
                    Log.m70e(str2, "Cannot reach WebViewUpdateService. " + e.getMessage());
                }
            }
        };
        try {
            ...
            String str = LOGTAG;
            Log.m69e(str, "error starting relro file creator for abi " + abi, t);
            crashHandler.run();
        }
    }
    ...
    static void reserveAddressSpaceInZygote() {
        ...
        boolean is64Bit = VMRuntime.getRuntime().is64Bit();
        long addressSpaceToReserve = is64Bit ? TrafficStats.GB_IN_BYTES : 136314880L;
        sAddressSpaceReserved = nativeReserveAddressSpace(addressSpaceToReserve);
        if (!sAddressSpaceReserved) {
            String str = LOGTAG;
            Log.m70e(str, "reserving " + addressSpaceToReserve + " bytes of address space failed");
        }
    }

    public static int loadNativeLibrary(ClassLoader clazzLoader, String libraryFileName) {
        if (!sAddressSpaceReserved) {
            Log.m70e(LOGTAG, "can't load with relro file; address space not reserved");
            return 2;
        }
        ...
        if (result != 0) {
            Log.m64w(LOGTAG, "failed to load with relro file, proceeding without");
        }
        return result;
    }
}
