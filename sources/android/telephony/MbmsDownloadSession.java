...
public class MbmsDownloadSession implements AutoCloseable {
        ...
        return MbmsUtils.startBinding(this.mContext, MBMS_DOWNLOAD_SERVICE_ACTION, new ServiceConnection() { // from class: android.telephony.MbmsDownloadSession.3
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                try {
                    ...
                    Log.m70e(MbmsDownloadSession.LOG_TAG, "Service died before initialization");
                    MbmsDownloadSession.sIsInitialized.set(false);
                } catch (RuntimeException e3) {
                    Log.m70e(MbmsDownloadSession.LOG_TAG, "Runtime exception during initialization");
            ...
            public void onServiceDisconnected(ComponentName name) {
                Log.m64w(MbmsDownloadSession.LOG_TAG, "bindAndInitialize: Remote service disconnected");
                MbmsDownloadSession.sIsInitialized.set(false);
    ...
    public void close() {
        ...
        try {
            try {
                ...
                Log.m68i(LOG_TAG, "Remote exception while disposing of service");
            }
            if (downloadService == null) {
                Log.m68i(LOG_TAG, "Service already dead");
            } else {
    ...
    private void writeDownloadRequestToken(DownloadRequest request) {
        ...
        if (token.exists()) {
            String str = LOG_TAG;
            Log.m64w(str, "Download token " + token.getName() + " already exists");
            return;
        }
        try {
    ...
    private void deleteDownloadRequestToken(DownloadRequest request) {
        ...
        if (!token.isFile()) {
            String str = LOG_TAG;
            Log.m64w(str, "Attempting to delete non-existent download token at " + token);
        } else if (!token.delete()) {
            String str2 = LOG_TAG;
            Log.m64w(str2, "Couldn't delete download token at " + token);
        }
    }
