...
public final class LocationAccessPolicy {
    ...
    private static void logError(Context context, LocationPermissionQuery query, String errorMsg) {
        if (query.logAsInfo) {
            Log.m68i(TAG, errorMsg);
            return;
        }
        Log.m70e(TAG, errorMsg);
        try {
    ...
    private static LocationPermissionResult checkAppLocationPermissionHelper(Context context, LocationPermissionQuery query, String permissionToCheck) {
        ...
        if (hasManifestPermission) {
            ...
            Log.m68i(TAG, query.callingPackage + " is aware of " + locationTypeForLog + " but the app-ops permission is specifically denied.");
    ...
    private static boolean isLocationModeEnabled(Context context, int userId) {
        ...
        if (locationManager == null) {
            Log.m64w(TAG, "Couldn't get location manager, denying location access");
            return false;
        }
        return locationManager.isLocationEnabledForUser(UserHandle.m110of(userId));
    }
