...
public class SystemSensorManager extends SensorManager {
    ...
    protected boolean registerListenerImpl(SensorEventListener listener, Sensor sensor, int delayUs, Handler handler, int maxBatchReportLatencyUs, int reservedFlags) {
        ...
        if (listener == null || sensor == null) {
            Log.m70e("SensorManager", "sensor or listener is null");
            return false;
        } else if (sensor.getReportingMode() == 2) {
            Log.m70e("SensorManager", "Trigger Sensors should use the requestTriggerSensor.");
            return false;
        } else if (maxBatchReportLatencyUs < 0 || delayUs < 0) {
            Log.m70e("SensorManager", "maxBatchReportLatencyUs and delayUs should be non-negative");
            return false;
            ...
            try {
                if (enable) {
                    ...
                    if (!isDataInjectionModeEnabled) {
                        Log.m70e("SensorManager", "Data Injection mode not enabled");
                        return false;
                    }
                    if (sInjectEventQueue == null) {
                        try {
                            ...
                            Log.m70e("SensorManager", "Cannot create InjectEventQueue: " + e);
                        }
                    }
                    if (sInjectEventQueue == null) {
                        z = false;
    ...
    protected boolean injectSensorDataImpl(Sensor sensor, float[] values, int accuracy, long timestamp) {
        synchronized (sLock) {
            if (sInjectEventQueue == null) {
                Log.m70e("SensorManager", "Data injection mode not activated before calling injectSensorData");
                return false;
            }
    ...
    private void cleanupSensorConnection(Sensor sensor) {
        ...
        if (sensor.getReportingMode() == 2) {
            synchronized (this.mTriggerListeners) {
                ...
                for (TriggerEventListener l : triggerListeners.keySet()) {
                    Log.m68i("SensorManager", "removed trigger listener" + l.toString() + " due to sensor disconnection");
        ...
        synchronized (this.mSensorListeners) {
            ...
            for (SensorEventListener l2 : sensorListeners.keySet()) {
                Log.m68i("SensorManager", "removed event listener" + l2.toString() + " due to sensor disconnection");
    ...
    public void updateDynamicSensorList() {
        synchronized (this.mFullDynamicSensorsList) {
            if (this.mDynamicSensorListDirty) {
                ...
                if (changed) {
                    Log.m68i("SensorManager", "DYNS dynamic sensor list cached should be updated");
            ...
            this.mDynamicSensorBroadcastReceiver = new BroadcastReceiver() { // from class: android.hardware.SystemSensorManager.2
                ...
                public void onReceive(Context context, Intent intent) {
                    if (intent.getAction() == Intent.ACTION_DYNAMIC_SENSOR_CHANGED) {
                        Log.m68i("SensorManager", "DYNS received DYNAMIC_SENSOR_CHANED broadcast");
    ...
    protected void registerDynamicSensorCallbackImpl(SensorManager.DynamicSensorCallback callback, Handler handler) {
        Log.m68i("SensorManager", "DYNS Register dynamic sensor callback");
        if (callback == null) {
    ...
    protected void unregisterDynamicSensorCallbackImpl(SensorManager.DynamicSensorCallback callback) {
        Log.m68i("SensorManager", "Removing dynamic sensor listerner");
    ...
    static final class TriggerEventQueue extends BaseEventQueue {
        ...
        protected void dispatchSensorEvent(int handle, float[] values, int accuracy, long timestamp) {
            ...
            if (t == null) {
                Log.m70e("SensorManager", "Error: Trigger Event is null for Sensor: " + sensor);
                return;
            }
            System.arraycopy(values, 0, t.values, 0, t.values.length);
            t.timestamp = timestamp;
