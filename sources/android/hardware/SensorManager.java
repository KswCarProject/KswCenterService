...
public abstract class SensorManager {
    ...
    public void unregisterListener(SensorEventListener listener) {
        if (listener != null) {
            unregisterListenerImpl(listener, (Sensor) null);
        }
    }
    ...
    public boolean registerListener(SensorEventListener listener, Sensor sensor, int samplingPeriodUs, int maxReportLatencyUs) {
        return registerListenerImpl(listener, sensor, getDelay(samplingPeriodUs), (Handler) null, maxReportLatencyUs, 0);
    }
    ...
    public SensorDirectChannel createDirectChannel(MemoryFile mem) {
        return createDirectChannelImpl(mem, (HardwareBuffer) null);
    }

    public SensorDirectChannel createDirectChannel(HardwareBuffer mem) {
        return createDirectChannelImpl((MemoryFile) null, mem);
    }
    ...
    public void registerDynamicSensorCallback(DynamicSensorCallback callback) {
        registerDynamicSensorCallback(callback, (Handler) null);
    }
    ...
    private static boolean remapCoordinateSystemImpl(float[] inR, int X, int Y, float[] outR) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (x == i5) {
                    fArr2[offset + i5] = sx ? -fArr[offset + 0] : fArr[offset + 0];
                }
                if (y == i5) {
                    fArr2[offset + i5] = sy ? -fArr[offset + 1] : fArr[offset + 1];
                }
                if (z == i5) {
                    fArr2[offset + i5] = sz ? -fArr[offset + 2] : fArr[offset + 2];
                }
                i4 = i5 + 1;
                i3 = 3;
            }
