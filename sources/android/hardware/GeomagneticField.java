...
public class GeomagneticField {
    ...
    public GeomagneticField(float gdLatitudeDeg, float gdLongitudeDeg, float altitudeMeters, long timeMillis) {
        ...
        while (n < MAX_N) {
            ...
            while (m2 <= n) {
                ...
                gcZ2 += relativeRadiusPower[n + 2] * m2 * ((sinMLon[m2] * g) - (cosMLon[m2] * h)) * legendre.f73mP[n][m2] * SCHMIDT_QUASI_NORM_FACTORS[n][m2] * inverseCosLatitude;
                gcZ -= ((((n + 1) * relativeRadiusPower[n + 2]) * ((cosMLon[m2] * g) + (sinMLon[m2] * h))) * legendre.f73mP[n][m2]) * SCHMIDT_QUASI_NORM_FACTORS[n][m2];
                m2++;
                MAX_N = MAX_N;
            }
            n++;
        ...
        this.f70mX = (float) ((gcX * Math.cos(latDiffRad)) + (gcY2 * Math.sin(latDiffRad)));
        this.f71mY = gcY;
        this.f72mZ = (float) (((-gcX) * Math.sin(latDiffRad)) + (gcY2 * Math.cos(latDiffRad)));
    }
    ...
    public float getDeclination() {
        return (float) Math.toDegrees(Math.atan2(this.f71mY, this.f70mX));
    }

    public float getInclination() {
        return (float) Math.toDegrees(Math.atan2(this.f72mZ, getHorizontalStrength()));
    }

    public float getHorizontalStrength() {
        return (float) Math.hypot(this.f70mX, this.f71mY);
    }

    public float getFieldStrength() {
        return (float) Math.sqrt((this.f70mX * this.f70mX) + (this.f71mY * this.f71mY) + (this.f72mZ * this.f72mZ));
    }
    ...
    private static class LegendreTable {
        ...
        public LegendreTable(int maxN, float thetaRad) {
            ...
            for (int n = 1; n <= maxN; n++) {
                this.f73mP[n] = new float[n + 1];
                this.mPDeriv[n] = new float[n + 1];
                for (int m = 0; m <= n; m++) {
                    if (n == m) {
                        this.f73mP[n][m] = this.f73mP[n - 1][m - 1] * sin;
                        this.mPDeriv[n][m] = (this.f73mP[n - 1][m - 1] * cos) + (this.mPDeriv[n - 1][m - 1] * sin);
                    } else if (n == 1 || m == n - 1) {
                        this.f73mP[n][m] = this.f73mP[n - 1][m] * cos;
                        this.mPDeriv[n][m] = ((-sin) * this.f73mP[n - 1][m]) + (this.mPDeriv[n - 1][m] * cos);
                    } else {
                        float k = (((n - 1) * (n - 1)) - (m * m)) / (((n * 2) - 1) * ((n * 2) - 3));
                        this.f73mP[n][m] = (this.f73mP[n - 1][m] * cos) - (this.f73mP[n - 2][m] * k);
                        this.mPDeriv[n][m] = (((-sin) * this.f73mP[n - 1][m]) + (this.mPDeriv[n - 1][m] * cos)) - (this.mPDeriv[n - 2][m] * k);
                    }
                }
            }
        }
