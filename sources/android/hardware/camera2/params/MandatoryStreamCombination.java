    ...
    public static final class Builder {
        ...
        public List<MandatoryStreamCombination> getAvailableMandatoryStreamCombinations() {
            if (!isColorOutputSupported()) {
                Log.m66v(MandatoryStreamCombination.TAG, "Device is not backward compatible!");
                return null;
            } else if (this.mCameraId < 0 && !isExternalCamera()) {
                Log.m68i(MandatoryStreamCombination.TAG, "Invalid camera id");
                return null;
            } else {
        ...
        private List<MandatoryStreamCombination> generateAvailableCombinations(ArrayList<StreamCombinationTemplate> availableTemplates) {
            ...
            if (availableTemplates.isEmpty()) {
                Log.m70e(MandatoryStreamCombination.TAG, "No available stream templates!");
                return null;
            }
            HashMap<Pair<SizeThreshold, Integer>, List<Size>> availableSizes = enumerateAvailableSizes();
            if (availableSizes == null) {
                Log.m70e(MandatoryStreamCombination.TAG, "Available size enumeration failed!");
                return null;
            }
            int i = 32;
            ...
            while (it.hasNext()) {
                ...
                if (isReprocessable) {
                    ...
                    if (combTemplate.mReprocessType == ReprocessType.PRIVATE) {
                        inputSize.add(maxPrivateInputSize2);
                        format = 34;
                    } else {
                        inputSize.add(maxYUVInputSize2);
                        format = 35;
                    }
                    int format2 = format;
                    streamsInfo.add(new MandatoryStreamInformation(inputSize, format2, true));
                    streamsInfo.add(new MandatoryStreamInformation(inputSize, format2));
                }
                ...
                while (i2 < length) {
                    ...
                    try {
                        ...
                        Log.m70e(MandatoryStreamCombination.TAG, "No available sizes found for format: " + template.mFormat + " size threshold: " + template.mSizeThreshold + " combination: " + combTemplate.mDescription);
                        return null;
                    }
                }
                ...
                try {
                    ...
                    Log.m70e(MandatoryStreamCombination.TAG, "No stream information for mandatory combination: " + combTemplate.mDescription);
                    return null;
                }
            }
        ...
        private HashMap<Pair<SizeThreshold, Integer>, List<Size>> enumerateAvailableSizes() {
            ...
            if (recordingMaxSize == null) {
                Log.m70e(MandatoryStreamCombination.TAG, "Failed to find maximum recording size!");
                return null;
            }
            ...
            if (previewSizes == null || previewSizes.isEmpty()) {
                Log.m70e(MandatoryStreamCombination.TAG, "No preview sizes within preview size bound!");
                return null;
            }
        ...
        private Size getMaxRecordingSize() {
            int quality = CamcorderProfile.hasProfile(this.mCameraId, 8) ? 8 : CamcorderProfile.hasProfile(this.mCameraId, 6) ? 6 : CamcorderProfile.hasProfile(this.mCameraId, 5) ? 5 : CamcorderProfile.hasProfile(this.mCameraId, 4) ? 4 : CamcorderProfile.hasProfile(this.mCameraId, 7) ? 7 : CamcorderProfile.hasProfile(this.mCameraId, 3) ? 3 : CamcorderProfile.hasProfile(this.mCameraId, 2) ? 2 : -1;
            if (quality < 0) {
                return null;
            }
        ...
        private Size getMaxCameraRecordingSize() {
            ...
            for (Size sz2 : videoSizes) {
                ...
                if (minFrameDuration > 3.3222591362126246E7d) {
                    Log.m68i(MandatoryStreamCombination.TAG, "External camera " + this.mCameraId + " has max video size:" + sz2);
                    return sz2;
                }
            }
            Log.m64w(MandatoryStreamCombination.TAG, "Camera " + this.mCameraId + " does not support any 30fps video output");
            return FULLHD;
        }
        ...
        private Size getMaxPreviewSize(List<Size> orderedPreviewSizes) {
            ...
            Log.m64w(MandatoryStreamCombination.TAG, "Camera " + this.mCameraId + " maximum preview size search failed with display size " + this.mDisplaySize);
