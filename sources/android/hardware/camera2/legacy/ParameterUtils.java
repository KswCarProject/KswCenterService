...
public class ParameterUtils {
    ...
    public static class WeightedRectangle {
        ...
        private static int clip(int value, int lo, int hi, Rect rect, String name) {
            if (value < lo) {
                Log.m64w(ParameterUtils.TAG, "toMetering - Rectangle " + rect + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + name + " too small, clip to " + lo);
                return lo;
            } else if (value > hi) {
                Log.m64w(ParameterUtils.TAG, "toMetering - Rectangle " + rect + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + name + " too small, clip to " + hi);
                return hi;
            } else {
                return value;
            }
    ...
    public static int getClosestAvailableZoomCrop(Camera.Parameters params, Rect activeArray, Size streamSize, Rect cropRegion, Rect reportedCropRegion, Rect previewCropRegion) {
        ...
        if (!actualCrop2.intersect(activeArray)) {
            Log.m64w(TAG, "getClosestAvailableZoomCrop - Crop region out of range; setting to active array size");
    ...
    public static MeteringData convertMeteringRectangleToLegacy(Rect activeArray, MeteringRectangle meteringRect, ZoomData zoomData) {
        ...
        if (!normalizedIntersected.intersect(NORMALIZED_RECTANGLE_DEFAULT)) {
            Log.m64w(TAG, "convertMeteringRectangleToLegacy - metering rectangle too small, no metering will be done");
    ...
    public static Face convertFaceFromLegacy(Camera.Face face, Rect activeArray, ZoomData zoomData) {
        ...
        if (leftEye != null && rightEye != null && mouth != null && leftEye.f59x != -2000 && leftEye.f60y != -2000 && rightEye.f59x != -2000 && rightEye.f60y != -2000 && mouth.f59x != -2000 && mouth.f60y != -2000) {
            Point leftEye2 = convertCameraPointToActiveArrayPoint(activeArray, zoomData, leftEye, true);
            Face api2Face = faceRect.toFace(face.f69id, leftEye2, convertCameraPointToActiveArrayPoint(activeArray, zoomData, leftEye2, true), convertCameraPointToActiveArrayPoint(activeArray, zoomData, leftEye2, true));
            return api2Face;
        }
    ...
    private static Point convertCameraPointToActiveArrayPoint(Rect activeArray, ZoomData zoomData, Point point, boolean usePreviewCrop) {
        Rect pointedRect = new Rect(point.f59x, point.f60y, point.f59x, point.f60y);
    ...
    private static WeightedRectangle convertCameraAreaToActiveArrayRectangle(Rect activeArray, ZoomData zoomData, Camera.Area area, boolean usePreviewCrop) {
        ...
        if (weight < 0) {
            Log.m64w(TAG, "convertCameraAreaToMeteringRectangle - rectangle " + stringFromArea(area) + " has too small weight, clip to 0");
        }
