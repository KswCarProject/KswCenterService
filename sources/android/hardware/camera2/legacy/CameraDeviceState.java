...
public class CameraDeviceState {
    ...
    public synchronized boolean setCaptureResult(final RequestHolder request, final CameraMetadataNative result, final int captureError, final Object captureErrorArg) {
        if (this.mCurrentState != 4) {
            Log.m70e(TAG, "Cannot receive result while in state: " + this.mCurrentState);
    ...
    private void doStateTransition(int newState, final long timestamp, final int error) {
        if (newState != this.mCurrentState) {
            ...
            Log.m68i(TAG, "Legacy camera service transitioning to state " + stateName);
        }
        ...
        switch (newState) {
            case 2:
                if (this.mCurrentState != 1 && this.mCurrentState != 3) {
                    Log.m70e(TAG, "Cannot call configure while in state: " + this.mCurrentState);
                    this.mCurrentError = 1;
                    doStateTransition(0);
                    return;
                }
            ...
            case 3:
                if (this.mCurrentState != 3) {
                    ...
                    Log.m70e(TAG, "Cannot call idle while in state: " + this.mCurrentState);
                    this.mCurrentError = 1;
                    doStateTransition(0);
                    return;
                }
            ...
            case 4:
                if (this.mCurrentState != 3 && this.mCurrentState != 4) {
                    Log.m70e(TAG, "Cannot call capture while in state: " + this.mCurrentState);
                    this.mCurrentError = 1;
                    doStateTransition(0);
                    return;
                }
