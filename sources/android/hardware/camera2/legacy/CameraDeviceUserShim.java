...
public class CameraDeviceUserShim implements ICameraDeviceUser {
    ...
    private static class CameraLooper implements Runnable, AutoCloseable {
        ...
        public int waitForOpen(int timeoutMs) {
            if (!this.mStartDone.block(timeoutMs)) {
                Log.m70e(CameraDeviceUserShim.TAG, "waitForOpen - Camera failed to open after timeout of 5000 ms");
                try {
                    this.mCamera.release();
                } catch (RuntimeException e) {
                    Log.m69e(CameraDeviceUserShim.TAG, "connectBinderShim - Failed to release camera after timeout ", e);
                }
    ...
    public void disconnect() {
        if (this.mLegacyDevice.isClosed()) {
            Log.m64w(TAG, "Cannot disconnect, device has already been closed.");
        }
        try {
    ...
    public SubmitInfo submitRequest(CaptureRequest request, boolean streaming) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot submit request, device has been closed.");
        ...
        synchronized (this.mConfigureLock) {
            if (this.mConfiguring) {
                Log.m70e(TAG, "Cannot submit request, configuration change in progress.");
    ...
    public SubmitInfo submitRequestList(CaptureRequest[] request, boolean streaming) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot submit request list, device has been closed.");
    ...
    public long cancelRequest(int requestId) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot cancel request, device has been closed.");
            throw new ServiceSpecificException(4, "Cannot cancel request, device has been closed.");
        }
        synchronized (this.mConfigureLock) {
            if (this.mConfiguring) {
                Log.m70e(TAG, "Cannot cancel request, configuration change in progress.");
    ...
    public boolean isSessionConfigurationSupported(SessionConfiguration sessionConfig) {
        if (sessionConfig.getSessionType() != 0) {
            Log.m70e(TAG, "Session type: " + sessionConfig.getSessionType() + " is different from  regular. Legacy devices support only regular session types!");
            return false;
        } else if (sessionConfig.getInputConfiguration() != null) {
            Log.m70e(TAG, "Input configuration present, legacy devices do not support this feature!");
            return false;
        } else {
            List<OutputConfiguration> outputConfigs = sessionConfig.getOutputConfigurations();
            if (outputConfigs.isEmpty()) {
                Log.m70e(TAG, "Empty output configuration list!");
                return false;
            }
            ...
            for (OutputConfiguration outputConfig : outputConfigs) {
                ...
                if (surfaceList.isEmpty() || surfaceList.size() > 1) {
                    Log.m70e(TAG, "Legacy devices do not support deferred or shared surfaces!");
                    return false;
                }
    ...
    public void beginConfigure() {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot begin configure, device has been closed.");
            throw new ServiceSpecificException(4, "Cannot begin configure, device has been closed.");
        }
        synchronized (this.mConfigureLock) {
            if (this.mConfiguring) {
                Log.m70e(TAG, "Cannot begin configure, configuration change already in progress.");
    ...
    public void endConfigure(int operatingMode, CameraMetadataNative sessionParams) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot end configure, device has been closed.");
            synchronized (this.mConfigureLock) {
            ...
            Log.m70e(TAG, "LEGACY devices do not support this operating mode");
            synchronized (this.mConfigureLock) {
                this.mConfiguring = false;
            }
    ...
    public void deleteStream(int streamId) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot delete stream, device has been closed.");
            throw new ServiceSpecificException(4, "Cannot delete stream, device has been closed.");
        }
        synchronized (this.mConfigureLock) {
            if (!this.mConfiguring) {
                Log.m70e(TAG, "Cannot delete stream, no configuration change in progress.");
    ...
    public int createStream(OutputConfiguration outputConfiguration) {
        int id;
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot create stream, device has been closed.");
            throw new ServiceSpecificException(4, "Cannot create stream, device has been closed.");
        }
        synchronized (this.mConfigureLock) {
            if (!this.mConfiguring) {
                Log.m70e(TAG, "Cannot create stream, beginConfigure hasn't been called yet.");
                ...
                Log.m70e(TAG, "Cannot create stream, stream rotation is not supported.");
    ...
    public void finalizeOutputConfigurations(int steamId, OutputConfiguration config) {
        Log.m70e(TAG, "Finalizing output configuration is not supported on legacy devices");
    ...
    public int createInputStream(int width, int height, int format) {
        Log.m70e(TAG, "Creating input stream is not supported on legacy devices");
        throw new ServiceSpecificException(10, "Creating input stream is not supported on legacy devices");
    }

    @Override // android.hardware.camera2.ICameraDeviceUser
    public Surface getInputSurface() {
        Log.m70e(TAG, "Getting input surface is not supported on legacy devices");
    ...
    public CameraMetadataNative createDefaultRequest(int templateId) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot create default request, device has been closed.");
        ...
        try {
            ...
            Log.m70e(TAG, "createDefaultRequest - invalid templateId specified");
    ...
    public CameraMetadataNative getCameraInfo() {
        Log.m70e(TAG, "getCameraInfo unimplemented.");
        return null;
    }
    ...
    public void waitUntilIdle() throws RemoteException {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot wait until idle, device has been closed.");
            throw new ServiceSpecificException(4, "Cannot wait until idle, device has been closed.");
        }
        synchronized (this.mConfigureLock) {
            if (this.mConfiguring) {
                Log.m70e(TAG, "Cannot wait until idle, configuration change in progress.");
    ...
    public long flush() {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot flush, device has been closed.");
            throw new ServiceSpecificException(4, "Cannot flush, device has been closed.");
        }
        synchronized (this.mConfigureLock) {
            if (this.mConfiguring) {
                Log.m70e(TAG, "Cannot flush, configuration change in progress.");
    ...
    public void prepare(int streamId) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot prepare stream, device has been closed.");
    ...
    public void tearDown(int streamId) {
        if (this.mLegacyDevice.isClosed()) {
            Log.m70e(TAG, "Cannot tear down stream, device has been closed.");
