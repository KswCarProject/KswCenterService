...
public class LegacyFaceDetectMapper {
            ...
            public void onFaceDetection(Camera.Face[] faces, Camera camera2) {
                ...
                synchronized (LegacyFaceDetectMapper.this.mLock) {
                    if (LegacyFaceDetectMapper.this.mFaceDetectEnabled) {
                        ...
                        Log.m72d(LegacyFaceDetectMapper.TAG, "onFaceDetection - Ignored some incoming faces sinceface detection was disabled");
                    }
                }
            }
        });
    ...
    public void processFaceDetectMode(CaptureRequest captureRequest, Camera.Parameters parameters) {
        ...
        if (fdMode != 0 && !this.mFaceDetectSupported) {
            Log.m64w(TAG, "processFaceDetectMode - Ignoring statistics.faceDetectMode; face detection is not available");
            return;
        }
        ...
        if (sceneMode == 1 && !this.mFaceDetectSupported) {
            Log.m64w(TAG, "processFaceDetectMode - ignoring control.sceneMode == FACE_PRIORITY; face detection is not available");
            return;
        }
        switch (fdMode) {
            case 0:
            case 1:
                break;
            default:
                Log.m64w(TAG, "processFaceDetectMode - ignoring unknown statistics.faceDetectMode = " + fdMode);
                return;
            case 2:
                Log.m64w(TAG, "processFaceDetectMode - statistics.faceDetectMode == FULL unsupported, downgrading to SIMPLE");
                break;
        }
        ...
        if (faces != null) {
            for (Camera.Face face : faces) {
                if (face != null) {
                    ...
                    Log.m64w(TAG, "mapResultFaces - read NULL face from camera1 device");
                }
            }
        }
