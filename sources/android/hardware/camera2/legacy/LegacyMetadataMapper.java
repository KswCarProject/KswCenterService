...
public class LegacyMetadataMapper {
    ...
    private static void mapScalerStreamConfigs(CameraMetadataNative m, Camera.Parameters p) {
        ...
        if (previewSizes.isEmpty()) {
            Log.m64w(TAG, "mapScalerStreamConfigs - failed to find any preview size matching JPEG aspect ratio " + jpegAspectRatio);
    ...
    private static void mapControlAf(CameraMetadataNative m, Camera.Parameters p) {
        ...
        if (afAvail == null || afAvail.size() == 0) {
            Log.m64w(TAG, "No AF modes supported (HAL bug); defaulting to AF_MODE_OFF only");
    ...
    private static void mapControlAwb(CameraMetadataNative m, Camera.Parameters p) {
        ...
        if (awbAvail == null || awbAvail.size() == 0) {
            Log.m64w(TAG, "No AWB modes supported (HAL bug); defaulting to AWB_MODE_AUTO only");
    ...
    private static int convertAntiBandingMode(String mode) {
        ...
        switch (z) {
            ...
            case true:
                return 3;
            default:
                Log.m64w(TAG, "convertAntiBandingMode - Unknown antibanding mode " + mode);
                return -1;
        }
    }
    ...
    static String convertAfModeToLegacy(int mode, List<String> supportedFocusModes) {
        if (supportedFocusModes == null || supportedFocusModes.isEmpty()) {
            Log.m64w(TAG, "No focus modes supported; API1 bug");
            return null;
        }
        String param = null;
        switch (mode) {
        ...
        if (!supportedFocusModes.contains(param)) {
            ...
            Log.m64w(TAG, String.format("convertAfModeToLegacy - ignoring unsupported mode %d, defaulting to %s", Integer.valueOf(mode), defaultMode));
            return defaultMode;
        }
        return param;
    }
