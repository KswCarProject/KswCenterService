...
public class LegacyRequestMapper {
    ...
    public static void convertRequestMetadata(LegacyRequest legacyRequest) {
        ...
        if (aberrationMode != 1 && aberrationMode != 2) {
            Log.m64w(TAG, "convertRequestToMetadata - Ignoring unsupported colorCorrection.aberrationMode = " + aberrationMode);
        }
        ...
        if (request.get(CaptureRequest.CONTROL_AWB_REGIONS) != null) {
            Log.m64w(TAG, "convertRequestMetadata - control.awbRegions setting is not supported, ignoring value");
        }
        ...
        if (aeFpsRange != null) {
            ...
            if (rangeToApply != null) {
                ...
                Log.m64w(TAG, "Unsupported FPS range set [" + legacyFps[0] + SmsManager.REGEX_PREFIX_DELIMITER + legacyFps[1] + "]");
            }
        }
        ...
        if (!compensationRange.contains((Range<Integer>) Integer.valueOf(compensation))) {
            Log.m64w(TAG, "convertRequestMetadata - control.aeExposureCompensation is out of range, ignoring value");
            compensation = 0;
        }
        ...
        if (focusDistance == null || focusDistance.floatValue() != 0.0f) {
            Log.m64w(TAG, "convertRequestToMetadata - Ignoring android.lens.focusDistance " + infinityFocusSupported + ", only 0.0f is supported");
        }
            ...
            switch (controlMode) {
                ...
                case 2:
                    ...
                    if (legacySceneMode != null) {
                        ...
                        modeToSet = "auto";
                        Log.m64w(TAG, "Skipping unknown requested scene mode: " + sceneMode);
                        break;
                    }
                default:
                    Log.m64w(TAG, "Control mode " + controlMode + " is unsupported, defaulting to AUTO");
                    modeToSet = "auto";
                    break;
            }
        ...
        if (params.getSupportedColorEffects() != null) {
            ...
            if (legacyEffectMode != null) {
                ...
                Log.m64w(TAG, "Skipping unknown requested effect mode: " + effectMode);
            }
        }
        ...
        if (testPatternMode != 0) {
            Log.m64w(TAG, "convertRequestToMetadata - ignoring sensor.testPatternMode " + testPatternMode + "; only OFF is supported");
        }
        ...
        if (location != null) {
            if (checkForCompleteGpsData(location)) {
                ...
                Log.m64w(TAG, "Incomplete GPS parameters provided in location " + location);
            }
        } else {
        ...
        if (sizes != null && sizes.size() > 0) {
            ...
            if (invalidSize) {
                Log.m64w(TAG, "Invalid JPEG thumbnail size set " + s + ", skipping thumbnail...");
            }
        ...
        if (mode != 1 && mode != 2) {
            Log.m64w(TAG, "convertRequestToMetadata - Ignoring unsupported noiseReduction.mode = " + mode);
        }
    }
    ...
    static int filterSupportedCaptureIntent(int captureIntent) {
        switch (captureIntent) {
            ...
            case 5:
            case 6:
                Log.m64w(TAG, "Unsupported control.captureIntent value 1; default to PREVIEW");
                break;
        }
        Log.m64w(TAG, "Unknown control.captureIntent value 1; default to PREVIEW");
        return 1;
    }
    ...
    private static List<Camera.Area> convertMeteringRegionsToLegacy(Rect activeArray, ParameterUtils.ZoomData zoomData, MeteringRectangle[] meteringRegions, int maxNumMeteringAreas, String regionName) {
        ...
        if (meteringRectangleList.size() == 0) {
            Log.m64w(TAG, "Only received metering rectangles with weight 0.");
            return Arrays.asList(ParameterUtils.CAMERA_AREA_DEFAULT);
        }
        ...
        if (maxNumMeteringAreas < i2) {
            Log.m64w(TAG, "convertMeteringRegionsToLegacy - Too many requested " + regionName + " regions, ignoring all beyond the first " + maxNumMeteringAreas);
        }
        ...
        if (aeMode == 1) {
            if (flashMode == 2) {
                if (ListUtils.listContains(supportedFlashModes, Camera.Parameters.FLASH_MODE_TORCH)) {
                    ...
                    Log.m64w(TAG, "mapAeAndFlashMode - Ignore flash.mode == TORCH;camera does not support it");
                }
            } else if (flashMode == 1) {
                if (ListUtils.listContains(supportedFlashModes, "on")) {
                    flashModeSetting = "on";
                } else {
                    Log.m64w(TAG, "mapAeAndFlashMode - Ignore flash.mode == SINGLE;camera does not support it");
                }
            }
        } else if (aeMode == 3) {
            if (ListUtils.listContains(supportedFlashModes, "on")) {
                ...
                Log.m64w(TAG, "mapAeAndFlashMode - Ignore control.aeMode == ON_ALWAYS_FLASH;camera does not support it");
            }
        } else if (aeMode == 2) {
            if (ListUtils.listContains(supportedFlashModes, "auto")) {
                flashModeSetting = "auto";
            } else {
                Log.m64w(TAG, "mapAeAndFlashMode - Ignore control.aeMode == ON_AUTO_FLASH;camera does not support it");
            }
        } else if (aeMode == 4) {
            if (ListUtils.listContains(supportedFlashModes, Camera.Parameters.FLASH_MODE_RED_EYE)) {
                ...
                Log.m64w(TAG, "mapAeAndFlashMode - Ignore control.aeMode == ON_AUTO_FLASH_REDEYE;camera does not support it");
            }
        }
    ...
    private static String convertAwbModeToLegacy(int mode) {
        switch (mode) {
            ...
            default:
                Log.m64w(TAG, "convertAwbModeToLegacy - unrecognized control.awbMode" + mode);
                return "auto";
        }
    }
    ...
    private static <T> T getIfSupported(CaptureRequest r, CaptureRequest.Key<T> key, T defaultValue, boolean isSupported, T allowedValue) {
        ...
        if (!isSupported) {
            if (!Objects.equals(val, allowedValue)) {
                Log.m64w(TAG, key.getName() + " is not supported; ignoring requested value " + val);
                return null;
            }
            return null;
        }
