...
final class LegacySensorManager {
    ...
    private static final class LmsFilter {
        ...
        /* renamed from: mV */
        private float[] f75mV = new float[24];

        /* renamed from: mT */
        private long[] f74mT = new long[24];
        private int mIndex = 12;

        public float filter(long time, float in) {
            LmsFilter lmsFilter = this;
            float v = in;
            float v1 = lmsFilter.f75mV[lmsFilter.mIndex];
            if (v - v1 > 180.0f) {
                v -= 360.0f;
            } else if (v1 - v > 180.0f) {
                v += 360.0f;
            ...
            lmsFilter.f75mV[lmsFilter.mIndex] = v;
            lmsFilter.f74mT[lmsFilter.mIndex] = time;
            lmsFilter.f75mV[lmsFilter.mIndex - 12] = v;
            lmsFilter.f74mT[lmsFilter.mIndex - 12] = time;
            float E = 0.0f;
            float D = 0.0f;
            float C = 0.0f;
            float B = 0.0f;
            ...
            while (i < 11) {
                ...
                float Z = lmsFilter.f75mV[j];
                float T = ((float) (((lmsFilter.f74mT[j] / 2) + (lmsFilter.f74mT[j + 1] / 2)) - time)) * 1.0E-9f;
                float dT = ((float) (lmsFilter.f74mT[j] - lmsFilter.f74mT[j + 1])) * 1.0E-9f;
                float dT2 = dT * dT;
                A += Z * dT2;
                B += T * dT2 * T;
                C += T * dT2;
